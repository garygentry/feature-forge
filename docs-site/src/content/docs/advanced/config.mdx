...
| `minRunnerVersion` | `0.14.0` | Minimum runner version required. |
...
- The navigator recommends `/clear` and a re-run before the next stage at **every** stage
  boundary on its own merits. It also measures the **current session's** token usage against
  the context window; past `contextWarnThreshold` (default 70%) that measurement *strengthens*
  the recommendation — a secondary signal, never the gate.
