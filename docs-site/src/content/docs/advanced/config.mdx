...
`forge.config.json` lives at the project root and is created by [forge-init](../pipeline/init.mdx) or `forge-bootstrap`. All keys are optional; the defaults below apply when a key is absent.
...
All of the runner command keys are templates with `{bin}`, `{backlogDir}`, `{specsDir}`, and `{iterations}` substituted at call time; most users never need to set these. See [Stage 5](../pipeline/stage-5-loop.mdx) for usage.
