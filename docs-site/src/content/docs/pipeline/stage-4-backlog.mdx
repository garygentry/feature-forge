...
Turns the spec suite into a validated `backlog.json` — a queue of self-contained work items the autonomous loop will implement one at a time. Each item must be self-contained because the loop runs with a [fresh context](../start-here/concepts.mdx) per item, so an item can't rely on memory from previous ones.
...
- `/feature-forge:forge-verify <feature>` — see [Verify & Fix](./verify-and-fix.mdx).
- `/feature-forge:forge-5-loop <feature>` — see [Stage 5 · Loop](./stage-5-loop.mdx).
