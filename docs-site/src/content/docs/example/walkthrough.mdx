...
Before you start, make sure the project is initialized — run [`/feature-forge:forge-init`](../pipeline/init.mdx) once if you haven't. You can check status at any point with [`/feature-forge:forge api-key-auth`](../pipeline/dashboard.mdx), and the [verify gate](../pipeline/verify-and-fix.mdx) is available after any stage.

## Stage 1 — PRD

Run the [PRD stage](../pipeline/stage-1-prd.mdx):
...
Run the [tech spec stage](../pipeline/stage-2-tech.mdx):
...
Run the [implementation specs stage](../pipeline/stage-3-specs.mdx):
...
Run the [backlog stage](../pipeline/stage-4-backlog.mdx):
...
Run the [loop stage](../pipeline/stage-5-loop.mdx):
...
Run the [docs stage](../pipeline/stage-6-docs.mdx):
...
At any point you can re-check progress with [`/feature-forge:forge api-key-auth`](../pipeline/dashboard.mdx) or run the [verify gate](../pipeline/verify-and-fix.mdx). For the bigger picture, see the [pipeline overview](../pipeline/overview.mdx) and [core concepts](../start-here/concepts.mdx).
