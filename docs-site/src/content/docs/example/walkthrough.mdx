...
Before you start, make sure the project is initialized — run [`/feature-forge:forge-init`](/pipeline/init/) once if you haven't. You can check status at any point with [`/feature-forge:forge api-key-auth`](/pipeline/dashboard/), and the [verify gate](/pipeline/verify-and-fix/) is available after any stage.
