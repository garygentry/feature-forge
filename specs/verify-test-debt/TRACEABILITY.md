...
| 4 | tech-spec §3.14 / §8.2: parametrize "is an established idiom in all three files"; `_VERB_INVOCATIONS` has 8 entries | `tests/test_state_verbs.py` has **zero** parametrize uses and **no `pytest` import**. `_VERB_INVOCATIONS` has **9** entries. Both measured. | `00` §10.5; `07` §1, §5.1 |
