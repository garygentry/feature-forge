...
- Step 4: [APPLIED] 2026-08-04 — V-005: `HANDOFF.md` rewritten as a post-loop record with an explicit header saying so; §1 now states 16/16 done with the green gate table, §2 keeps the stall diagnosis and its resolution as history, and the process-gaps table survives (renumbered §3, #195 marked resolved). V-012: `backlog.json.bak` diffed and confirmed strictly stale (one item `in_progress`/`completedAt: null` where the live file has it `done`) — exactly the confusion risk V-012 describes. **This entry was WRONG when first written and is corrected here rather than overwritten.** It claimed the file was removed. It was not: the `rm` hit an interactive `rm -i` alias, read EOF, treated it as "no", and exited 0, so the `&& echo` fired on a deletion that never happened — and because `**/backlog.json.bak` had been in `.gitignore` since `e02bd4e`, `git status` read clean throughout and hid the miss. The claim was recorded without verifying the postcondition. The scoped re-verify (round 2) caught it and reported V-012 UNRESOLVED.
- Step 4 (completion): [APPLIED] 2026-08-04 — `specs/verify-test-debt/backlog.json.bak` actually deleted with `/bin/rm -f`, bypassing the alias, and the postcondition verified two ways: `ls` reports no such file, and `git status --porcelain --ignored specs/verify-test-debt/` shows no `.bak` entry. The redundant `*.json.bak` rule this pass had added to `.gitignore` was **removed** (re-verify NEW-1): the pre-existing `**/backlog.json.bak` at line 10 already covers this path, so the addition bought nothing and only widened the set of artifacts that can hide from `git status`. `.gitignore` is therefore back to its committed state, which also makes `01` §2's `.gitignore` inventory line accurate again with no edit.
...
### Round-2 re-verify — advisory items from the fix delta

A scoped clean-room re-verify confirmed V-001..V-014 against their acceptance evidence and
independently reproduced every gate figure claimed above with **no discrepancy**, including
the V-007 non-vacuity result (4 of 5 fail under a short-circuited `read_allowlist_file`;
the survivor is the test that deliberately uses no allowlist). It reported one unresolved
prior finding — V-012, corrected above — and three advisory observations, all of them
artifacts this fix pass itself produced:

- **NEW-1** (`improvement`) — the `*.json.bak` rule added to `.gitignore` was redundant with
  the pre-existing `**/backlog.json.bak`. **Applied:** rule removed.
- **NEW-2** (`inconsistency`) — `01` §1's new "complete file-ownership map" claim did not
  cover the documentation and test surfaces this fix pass added. **Applied:** §1 gained a
  "Scope of this map" paragraph bounding the inventory to the implementation surface and
  naming where post-verify additions are recorded instead.
- **NEW-3** (`inconsistency`) — `HANDOFF.md` §1's gate table, written by this pass and
  presented as current, carried the pre-fix figures (1797/1799). **Applied:** updated to
  1802/1804, with the 1799 loop-completion figure retained as history.
