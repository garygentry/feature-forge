...
**RESOLVED 2026-07-31 — (b) Canon + test.** The user chose the canon amendment. Step 3 therefore edits `skills/forge-{1-prd,2-tech,3-specs,4-backlog}/SKILL.md` and `references/shared-conventions.md` § Verify Capability so the affirmative choice is stated to **dispatch**, pins `"dispatched on the affirmative"` as c1b uniformly on all six surfaces, and regenerates the adapters. No per-surface phrasing is admitted.
...
- Step 1: [APPLIED] 2026-07-31 — Replaced the whole-file substring search at `tests/test_capability_determination_prose.py` with an `ast`-based structural check: parse `Path(__file__)`, require exactly one module-level `ALL_SURFACES` `AnnAssign`, and assert its value is a `Call` to `_capability_surfaces`. Added `import ast`. Deleted the false parenthetical "(Unlike the vacuous form, this reads for a DIFFERENT string than the one it writes.)" and replaced it with a statement of why the `ast` form is used — a substring search would be satisfied by the assertion's own source line, which is the V-002 defect. **Acceptance (roster-PRESERVING mutation):** on a hardlinked copy, `:316` replaced with a hand-kept list comprehension naming the *same six* paths read from disk → `test_the_controls_cover_every_determining_surface` **RED at `:459`**, the derivation assertion (`isinstance(<ast.ListComp>, ast.Call)` is False). The floor assertion did **not** fire — the roster stayed at 6. Unmutated copy: 37 passed.
- Step 2: [APPLIED] 2026-07-31 — Corrected the stale sub-clause counts and guard banners in the same file: the `CLAUSES` lead-in dropped "three"; the `"choice 2 omitted"` note, `_assert_clauses_in`'s docstring, and control 3a's docstring were re-counted; the Guard 1 banner became "states every clause" and the Guard 2 banner "the negative controls spec 07 §6.2 mandates (control 3 split per sub-clause)". Read the whole `CLAUSES` comment block and both banners end-to-end afterwards. (Step 3 then advanced the same counts from three to four — see below.)
- Step 3: [APPLIED] 2026-07-31 — **Decision 1 resolved (b): canon + test.** Canon: the shared capability sentence in `skills/forge-{1-prd,2-tech,3-specs,4-backlog}/SKILL.md` and `references/shared-conventions.md` § Verify Capability now states that on *Verify now* the clean-room `forge-verifier` is **dispatched on the affirmative choice**, never merely printed for the user to run later. Tests: `CLAUSES["c1"]` split into `c1a` (gate) and `c1b` (dispatch, `("dispatched on the affirmative",)`); control 3a split into 3a-i (c1a) and 3a-ii (`test_downgrading_the_affirmative_choice_to_a_printed_command_fails_the_guard`), each parametrized over all six surfaces with its own `pytest.raises(match=r"clause \(c1[ab]\)")`; the module docstring's clause-(c) paragraph now describes four sub-clauses and records both merge failures. Adapters regenerated. **Acceptance:** the dispatch→print misreading goes **RED on all six** surfaces — `forge-1-prd`, `forge-2-tech`, `forge-3-specs`, `forge-4-backlog` each fail on `clause (c1b)`; `forge-verify` and `forge-fix` likewise on `clause (c1b)`. Deleting the gate half goes **RED on `clause (c1a)`** on `forge-1-prd`, `forge-verify`, and `forge-fix`. Baseline 43 passed.
  - **Deviation (Step 3, disclosed):** `c1a` accepts a **third** phrasing, `"presented through the Step 6 gate"`. `forge-fix` writes the gate half that way — it has two numbered steps a directive could route through, so it names which. Without it, `c1a` matched `forge-fix` on nothing and the module went red on six tests at baseline. Same obligation, one word wider; deleting it still turns `forge-fix` red on `clause (c1a)` (measured above). This is a phrasing the surface genuinely uses, not a mechanism token.
  - **Deviation (Step 3, disclosed):** V-003's acceptance mutation (i) as literally written — relabelling `*Verify now* (recommended)` to `*Print the verify command for the user to run later* (recommended)` and changing nothing else — stays **GREEN**, and that is correct under option (b) rather than a miss. The finding wrote that mutation against the *old* text, where the option label was the only token carrying anything dispatch-shaped. After the canon amendment the obligation is stated in its own clause of the sentence, so relabelling the option leaves "the clean-room `forge-verifier` is **dispatched on the affirmative choice**" intact: the prose becomes self-contradictory but the obligation is still said, which is what `c1b` exists to check. Pinning the option label too would re-admit a gate-SHAPE token — the `"choice 2 omitted"` mistake Decision 1 chose (b) to avoid. The semantically equivalent mutation (rewrite the clause so the affirmative choice prints) is the one recorded above, and it goes red on all six.
- Step 4: [APPLIED] 2026-07-31 — Added `assert CALL_SPAN <= 3` immediately before `assert LOOKAHEAD <= CALL_SPAN` in `tests/test_state_verb_call_sites.py`, with a message naming the widening path. Extended the `CALL_SPAN` constant comment to record that it is load-bearing for Guard 1's window, not only for call flattening, and extended the pin docstring to explain why three assertions are needed rather than two. **Acceptance:** on a copy, `CALL_SPAN = 8` and `LOOKAHEAD = 8` → `test_the_window_is_no_wider_than_the_measured_maximum` **RED at `:183`**, the new `CALL_SPAN` assertion. Green before the change; baseline 10 passed.
- Step 5: [APPLIED] 2026-07-31 — Kept `assert live <= annotated[rel]` as the hard gate and added a non-fatal drift report: entries where `live < annotated[rel]` are collected and emitted once via `warnings.warn`, naming each as `{rel}: annotated {N}, live {M}`. Extended the maintenance contract at `scripts/check-spec-purity.py` to state that an annotation may only be **lowered** to a freshly re-derived count, never raised except in a pattern-widening change, and that the warning is the instruction to lower it. No annotation value changed — all 29 are exact at HEAD, so the baseline run emits **no** warning. **Acceptance:** inflating `"eval/README.md",  # 1` to `# 9999` leaves the hard gate **GREEN** (correct — a ceiling is blind upward by design) and emits `UserWarning: … eval/README.md: annotated 9999, live 1`; lowering `"scripts/build-adapters.py",  # 83` to `# 2` still goes **RED** on "grandfathered debt may only shrink".
- Step 6: [APPLIED] 2026-07-31 — `python3 scripts/build-adapters.py` (Step 3 touched canon), then `--check` exit **0**. `bash scripts/validate.sh` run 2 exit **0**, run 3 exit **0** (back-to-back), both `All checks passed!`, both **1808 passed / 2 skipped**. Delta accounted for control-by-control: 1808 − 1802 = **+6**, exactly the six parametrizations of the new control 3a-ii (one per determining surface); Steps 1, 2, 4 and 5 modified existing tests and added no test functions. `find tests/fixtures -name '__pycache__' -o -name '*.pyc' | wc -l` = **0** after every run. `ruff check scripts/ eval/` clean; `python3 scripts/check-spec-purity.py` exit 0, `spec-purity: PASS — 0 violations`; `ruff check tests/ --select F841,F541` clean; `ruff check tests/` **19 errors** (unchanged).

**Mutation discipline note.** Every acceptance mutation above was run on a **hardlinked** copy of `tests/`, `skills/`, `references/`, `scripts/`, `eval/` and `adapters/` (`cp -al`), with the mutated file unlinked before rewriting so the shared inode is never truncated. The repository itself was not modified by any mutation, and each red is recorded with the **line number and clause id** it fired on, not merely as "the test failed".
