...
> **All six decisions RESOLVED 2026-08-01** by the user, each at the report's recommended option:
> **D1 → (b)** keep `CALL_SPAN = 3`, restate the comment · **D2 → (a)** spec + code + regression test ·
> **D3 → leave** the two legacy entries · **D4 → (a)** record the deliberate non-pin ·
> **D5 → fold V-012 into Step 7** · **D6 → keep `smokeCommand: null`**, re-record the narrowed advisory.

**Decision 1 (V-001) — correct `CALL_SPAN` to its measured value, or restate what it measures.** ✅ **RESOLVED → (b)**. The two options are not equivalent and the applier should not pick:
...
**Decision 2 (V-004) — apply the classifier fix now, or spec-only and defer the code change.** ✅ **RESOLVED → (a)**. The fix changes runtime classification: stages sitting at `findings-applied` will begin reading `stale` instead of `fresh`, so the navigator's "verify before continuing" gate will start firing for them. That is the spec'd intent, but it is a live behaviour change for any feature currently parked at `findings-applied`, **including `forge-bootstrap` and `epic-orchestration` in this repo**.
...
**Decision 3 (V-004) — this feature's own two legacy entries.** ✅ **RESOLVED → leave them.** `forge-verify-specs` and `forge-verify-backlog` in `.pipeline-state.json` carry `verifiedStageVersion` alongside `findings-applied`. Do **not** hand-edit them — `state-verify` is the sole writer and `findings-applied` is not re-writable to drop a key. Either leave them (the new guard makes them harmless) or resolve them by an actual re-verify of the specs and backlog stages. Recommendation: **leave them**; the guard neutralises them, and re-verifying two long-settled stages is out of scope for an impl fix pass.

**Decision 4 (V-008) — record the non-pin, or pin the label separately.** ✅ **RESOLVED → (a) record.** Recommendation: **record** (option (a)), which V-002's fix largely writes anyway. The round-4 deviation establishing that the label cannot be pinned uniformly is sound and must not be reversed.

**Decision 5 (V-012) — fix now, or defer.** ✅ **RESOLVED → fold into Step 7.** V-012 is pre-existing and outside `git diff 21f1c34..HEAD`; the reshaping touches the live stage-exit route. Recommendation: **fold it into V-004's Step 7**, since V-004's fix already requires mirroring the guard into `_verify_state_for` and that is only meaningful if the helper is on the runtime path. Doing them separately means touching the same two functions twice.

**Decision 6 (V-013) — configure a `smokeCommand` or keep it `null`.** ✅ **RESOLVED → keep `null`.** Recommendation: **keep `null`** and re-record the narrowed advisory. Do not set it to `bash scripts/validate.sh`.
...
- Decisions 1-6: [RESOLVED] 2026-08-01 — all six taken at the report's recommended option (D1→b, D2→a, D3→leave, D4→a, D5→fold into Step 7, D6→keep null).
- Step 1: [APPLIED] 2026-08-01 — module docstring clause (c) rewritten: the "measured twice" second half now describes the *dispatch clause* rewrite (RED on all six) instead of the option relabel; added a paragraph recording the deliberate non-pin of the affirmative option label, with the reason (forge-verify/forge-fix render no option label at all). Ground truth re-confirmed first: (i) `*Verify now*` count 0/0 in forge-verify/forge-fix, 1 each in the four authoring stages; (ii) relabelling all four authoring stages → 43 passed GREEN; (iii) neutering `CLAUSES["c1b"]` → RED on `clause (c1b)` on all six control-base surfaces plus shared-conventions.md. Docstring re-read end-to-end against control 3a-ii's; both now tell the same story. (V-002, V-008 option (a))
- Step 2: [APPLIED] 2026-08-01 — swept the residual two-way "half" framing to "obligation" at all remaining sites (module docstring c1a/c1b/c2/c3, the `CLAUSES["c1a"]` comment, and the four control docstrings incl. "This is the obligation that had NO pin of its own"). `grep -n half` over the module now returns nothing. `tests/test_state_verb_call_sites.py:176` ("both halves of the window") deliberately left alone — genuinely two-sided. 43 passed; `ruff check tests/` still 19; no new E501 in the touched file. (V-005)
- Step 3: [APPLIED] 2026-08-01 — roster-derivation guard rebuilt on the binding-count form; both `_capability_surfaces` alias assertions added; comment extended with the decoy rationale; `V-002` → `round-3 V-002` and the `CLAUSES` comment's `(V-001)` → `(round-3 V-001)`. **Acceptance (PYTHONDONTWRITEBYTECODE=1, -p no:cacheprovider, `__pycache__` purged, fresh copy per probe) — all six probes red at exactly one assertion, none at the floor (:470); unmutated 43 passed.** Line map: :512 binding-count (new) · :516 AnnAssign-shape (new) · :520 derivation `Call` (existing) · :527 FunctionDef alias (new) · :532 re-bind alias (new). De-shifted reds: P1 roster-**preserving** hand-kept `ast.List` → :520 · P2 differently-named hand-kept fn → :520 · P3 `sorted(_capability_surfaces())` → :520 · P4 `AnnAssign`→`Assign` still derived → :516 · **P5 decoy kept + `ALL_SURFACES` re-bound → :512 (was GREEN)** · **P6 `_capability_surfaces = _hand_kept_surfaces` → :532 (was GREEN)**. (V-007, V-010)
  - **Deviation (disclosed):** V-007's literal suggested snippet accepts a plain `ast.Assign` as the single binding, which would have turned probe P4 GREEN — contradicting the same finding's acceptance criterion that "the four already-red probes must stay red". Kept the annotated-assignment requirement as its own assertion (:516) *in addition to* the prescribed binding count (:512), so the fix is strictly stronger than the snippet and every probe in V-007's table still bites. No `CLAUSES` entry was touched.
- Step 4: [APPLIED] 2026-08-01 — **Decision 1(b)**. `CALL_SPAN` stays 3; its stated basis is now true. The constant comment no longer claims to cover "the longest call in canon" — it records that canon's max span is **4** and names all six four-line sites it truncates **by design**, then states the real measured basis: every `--status skipped` `SKIP_STATUS_RE` searches for sits at offset **0-1** from its verb line (forge-5-loop:263 → 0; forge-4-backlog:172 → 1; forge-6-docs:53 → 1), so 3 is 1 plus two lines of margin. The pin docstring drops "a maintainer who adds a fenced call with three flag lines" (canon already has six) and names the real trigger to re-measure: *a searched flag appearing on a fourth line*. The assertion message now names that invariant instead of the call length. `_state_verify_call_text`'s docstring corrected from "a `\`-continued line **pair**" to "up to `CALL_SPAN` lines", with the truncation stated — the module now gives ONE number for the quantity. **Acceptance:** independent walker re-derived the span histogram `{1:4, 2:11, 3:13, 4:6}` over 34 sites (= `MIN_CALL_SITES`), max **4** at exactly the six sites the finding named, and max searched-flag offset **1** — every number written into the comment equals its measurement. All three pins still bite independently at their own lines: `LOOKBEHIND = 20` → RED :199 · `CALL_SPAN = 8` → RED :204 · `LOOKAHEAD = 4` → RED :211 · `CALL_SPAN = 8` + `LOOKAHEAD = 8` → RED :204. 10 passed; ruff clean. (V-001)
- Step 5: [APPLIED] 2026-08-01 — deleted `stacklevel=2,` from the `CITATION_GRANDFATHERED` drift warning, leaving the default, and added the one-line comment explaining why (it warns about its own module's data, not on behalf of a caller). **Acceptance (measured, path recorded):** on a scratch copy with `"eval/README.md",  # 1` inflated to `# 9999`, the pytest warnings-summary location line now reads `tests/test_check_spec_purity.py:462: UserWarning: CITATION_GRANDFATHERED annotations read high …` and echoes the `warnings.warn(` source line — **not** `_pytest/python.py:166`, which is what it printed before. The test still passes (33 passed, 1 skipped, 1 warning). Unmutated repo copy emits **0** warnings and 34 passed. `ruff check tests/` unchanged at 19; the single error in this file is the pre-existing E501 at `:273`, unshifted (the edit is below it). (V-009)
- Step 6: [APPLIED] 2026-08-01 — `03-verification-state.md` §5.1 gained the seventh classifier rule, inserted before the generic resolved/version handling and after the `build_rows` bullet: a `findings-applied` entry never classifies `fresh` regardless of any `verifiedStageVersion` it carries, citing §4.2 step 4, REQ-DEBT-06 for the legacy path, §3.3 for the write-side counterpart, and REQ-DEBT-05. The six existing bullets were neither renumbered nor reworded. (V-004)
- Step 7: [APPLIED] 2026-08-01 — **Decisions 2(a) + 5**. Guard added in `verify_state` (`scripts/forge-session.py`), in the shared `_classify_verify_entry` (which is the ONE home both stage-exit callers funnel into, so the guard has one implementation rather than three), and mirrored in `epic-manifest.py`'s `_epic_verify_state`; that reader's docstring no longer says `findings-applied` lands in `stale` "in practice" (writer discipline) but UNCONDITIONALLY. V-012 folded in: the stage-exit route's non-epic branch now reads `verify_label = _verify_state_for(state, route_stage)` instead of re-deriving the token lookup + classify inline; the epic branch keeps its direct `_classify_verify_entry` call with the manifest revision (REQ-SEC-01). **`_verify_state_for`'s signature is unchanged** — `tests/test_auto_verify.py:486` pins it and §5.1 says "without changing their signatures", so the optional epic-context parameter V-012 floated was NOT added; the report's own alternative (non-epic branch calls the helper) achieves the call-path fix without touching four spec quotations. **Acceptance (call-path, not suite-green):** the executable-corpus AST orphan sweep over 10 files now returns **NONE** — `_verify_state_for` is no longer orphaned and has a real call at `:3547`. Mutating its body to `return "fresh"` reds a genuine **CLI-subprocess** test at `tests/test_stage_exit.py:134` (`assert d["verifyState"] == "never"` → got `'fresh'`), not merely `tests/test_auto_verify.py`. Unmutated: `tests/test_stage_exit.py` + `tests/test_auto_verify.py` = 590 passed; `ruff check scripts/ eval/` clean. (V-004, V-012)
  - **Note on scope:** `verify_entry`/`verify_current` are still resolved in both branches — they feed `_debt_metadata_warnings` at `:3804`, outside the range the finding quoted. An intermediate edit that dropped them raised `NameError` in the CLI; caught by the same subprocess test and restored.
- Step 8: [APPLIED] 2026-08-01 — added `test_legacy_findings_applied_carrying_a_version_still_reads_stale` to `tests/test_auto_verify.py`. It builds a `findings-applied` entry carrying `verifiedStageVersion: 1` against `forge-1-prd.version = 1` and asserts `verify_state` → `("forge-1-prd", "stale")`, `pending_verify` → `"forge-1-prd"`, and `_verify_state_for` → `"stale"`. The docstring states in capitals that the shape is unreachable through the current writer (`_write_verify_entry` omits the key and refuses `--verified-stage-version` on that status) and arrives only via legacy state under REQ-DEBT-06, so it cannot later be "simplified" into a vacuous writer-behaviour assertion. **Acceptance:** against a pre-Step-7 scratch copy (guards reverted, test kept) it **FAILS** — `assert ('forge-1-prd', 'fresh') == ('forge-1-prd', 'stale')`, the exact defect V-004 named. Post-fix: 84 passed. **Live demonstration re-run on this repo's own `.pipeline-state.json`**, truncated so specs is the latest completed production stage: previously `('forge-3-specs', 'fresh')`, now `('forge-3-specs', 'stale')` with `pending_verify` → `forge-3-specs`. Per Decision 3 the two legacy entries were left untouched; the guard neutralises them. (V-004)
- Step 9: [APPLIED] 2026-08-01 — the run-on split into two sentences on all five canon surfaces, one wording adopted across them. The four authoring stages and `references/shared-conventions.md` now read "…leaving exactly two choices: *Verify now* (recommended) and *Skip for now*. The clean-room `forge-verifier` is **dispatched on the affirmative choice**, never merely printed for the user to run later; *Skip for now* is persisted as an explicit `skipped` before any advancing block." This removes the circular "on which … on the affirmative choice", removes the comma-series that made "the latter" reach past two clauses, and closes the gratuitous "rather than" / "never merely" split — `shared-conventions.md` now uses the skills' wording verbatim. `dispatched on the affirmative` and `reuse the Standard Verify Gate block for consent` are intact and inside the same blank-line-delimited paragraph. Adapters regenerated. **Acceptance:** (1) `c1a`/`c1b` still match in-paragraph on all six — 43 passed; (2) the dispatch→print downgrade goes **RED at `clause (c1b)` on all six** control-base surfaces; (3) `build-adapters.py --check` exit **0**; (4) all **72/72** `adapters/**/shared-conventions.md` mirrors carry the amended sentence **exactly once**. Adapter + prose + protocol guards: 311 passed. (V-006)
  - **Correction to the round-5 report's CHECK-I10 / Step-9 acceptance wording:** "all 72 `shared-conventions.md` mirrors byte-identical to canon" is not an achievable check and was already false at HEAD — measured **60/72** byte-identical both **before** and **after** this change. The 12 that differ are the entire `adapters/pi/` tree, differing only by the documented host command-surface degradation (`/feature-forge:forge-1-prd` → `/skill:forge-1-prd`), which `tests/_forge_paths.py` records as expected for adapter copies. The meaningful invariant — every mirror carrying the amended sentence exactly once — holds at 72/72, and the byte-identical count is unchanged by this pass.
  - **`backlog.json` item 017** still contains "the latter persisted as an explicit `skipped`" in its own description. Left as-is: it is a completed work record paraphrasing the requirement, and the amended canon states that requirement more explicitly, so its AC remains satisfied. The other two hits are round-3 and round-5 verification reports quoting the pre-amendment wording as historical evidence, which must not be rewritten.
- Step 10: [APPLIED] 2026-08-01 — `01-architecture-layout.md` §2 "Complete File Layout" restored to complete: all seven missing rows added in their own blocks with each block's own marker column (scripts/references at col 43, eval/tests at col 44, measured from the existing rows rather than assumed) — `scripts/check-spec-purity.py` **M**, `references/forge-config-schema.json` **M**, `tests/test_capability_determination_prose.py` **N**, `tests/test_gate_pytest_reachability.py` **N**, `tests/test_state_verb_call_sites.py` **M**, `tests/test_check_spec_purity.py` **M**, and a top-level `README.md` **M**. `.gitignore` and `forge.config.json` deliberately NOT added (out-of-band commit `b3110b1`, not this feature). `07-testing-strategy.md` now names both new guard modules: the capability module is attached to §6.2's capability bullet (the bullet that specified it but never named it), and the gate module to §8.2 beside the PASS/SKIP discussion it guards. Both one-liners were written **after reading each module's docstring and assertions**, not from the finding's summary — the gate module's description was corrected on that read from "reaches its pytest invocation" to what it actually pins (pytest importable via the same `find_spec` resolution `validate.sh` gates on, plus the SKIP branch staying distinguishable and incrementing `WARNINGS`). (V-003)
- Step 11: [APPLIED] 2026-08-01 — the standing-invariant note's opening sentence corrected from "(30 items, 001-030)" to "(32 items, 001-032; 031-032 appended by forge-5-loop as fix items after backlog verification)". Live artifact re-counted first: `backlog.json` holds **32** items, ids `001`..`032`. Written through `state-note` only — never a hand-edit — re-emitting the **entire** combined string in one `--note` because the field has overwrite semantics; exit 0, length 1156 → 1230, opening clause replaced and the tail ("…retain it, do not re-add it.") verified intact. No `--epic` passed: `stage-exit-coverage` resolves flat with no `epic` back-pointer, so it is standalone. (V-011)
- Step 13 (not in the plan, forced by the gate): [APPLIED] 2026-08-01 — `scripts/check-spec-purity.py` went **FAIL — 1 violation** on Step 7's new comment, which cited `03 §5.1, 02 "Private helpers"`; `_SPEC_CITATION_RE` matches `\b0[0-9]\s*§` and `scripts/forge-session.py` is deliberately NOT grandfathered. Rewrote the comment to be self-contained ("`_verify_state_for`, the designated stage-exit routing classifier"). Bare `§4.2`/`§5.1` references carry no `0N` prefix and are not matched, so the remaining guard comments stand. `check-spec-purity.py` back to **PASS — 0 violations**.
- Step 12: [APPLIED] 2026-08-01 — regenerated and re-gated. `df -h /` checked **before** gating per measurement hazard 2 (3.2 GB free, above the ~1 GB the suite needs; 2.6 GB after). `build-adapters.py` then `--check` exit **0**. `bash scripts/validate.sh` run **twice genuinely back-to-back**: exit **0** both times, `All checks passed!` both times, **1809 passed / 2 skipped** both times, `PASS: epic-manifest pytest suite` (not SKIP) both times, and `find tests/fixtures -name '__pycache__' -o -name '*.pyc' | wc -l` = **0** after each. **Suite delta re-derived by node-ID SET DIFFERENCE, not from totals** (via a `git worktree` at HEAD — note `git archive` cannot be used for this: `.gitattributes` marks `tests/`, `specs/` and `eval/` `export-ignore`, so an archive-based baseline silently collects zero tests): 1810 → 1811 collected, **added set is exactly `tests/test_auto_verify.py::test_legacy_findings_applied_carrying_a_version_still_reads_stale`**, **removed set empty** — nothing lost or renamed. The delta is +1 rather than +5 because Decision 4 took option (a), which records the non-pin in prose and adds no test. `ruff check scripts/ eval/` clean; `ruff check tests/` **19** (unchanged); `ruff check tests/ --select F841,F541` clean; `check-spec-purity.py` PASS — 0 violations. 124 files changed: 108 adapter mirrors + 16 non-adapter.
