...
- Step 1: [APPLIED] 2026-08-01 — Rewrote the recorded-decision comment's out-of-scope justification (`tests/test_capability_determination_prose.py:574-596`), prose only, no assertion changed (Decision 1(c) stands). Removed the false "caught by the derivation-`Call` assertion below regardless of binding form" clause; the comment now states the count assertion reds any ADDITIONAL counted binding (`Assign`/`AnnAssign`/`AugAssign`) and the derivation-`Call` assertion reds a literal VALUE at the single annotated binding, while a literal installed through an out-of-scope form is NOT caught (it leaves the real derivation as `bindings[0]`) — the recorded, accepted residue. Moved `del` out of the "leave the suite green with the roster displaced" set into a separate note (it UNBINDS: raises `NameError` at collection when placed before the reads, leaves the captured roster intact when after). Added `match`-capture (`case ALL_SURFACES`) to the enumerated out-of-scope forms. **Acceptance evidence:** literal-decoy matrix re-run in-process — walrus / `for` target / `with … as` / `global`-in-fn / `match`-capture all leave the five-assertion guard GREEN with the runtime roster = the literal; counted `Assign` control reds via the count assertion. Amended comment read end-to-end against the five assertions; no remaining sentence attributes to the derivation-`Call` assertion a catch it does not have. `validate.sh` → `All checks passed!` (1809 passed / 2 skipped); `ruff check tests/` = 19 (unchanged); `check-spec-purity.py` PASS; `ruff check scripts/ eval/` clean.
- V-002 (advisory, CHECK-I21 `smokeCommand: null`): no action — keep `null` per Decision 6 / §8.3. No step.
