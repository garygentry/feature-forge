...
**All resolved 2026-07-31** during `/feature-forge:forge-fix --served-stage forge-5-loop`. Resolutions are recorded inline below; each names the step that carries it.

| # | Resolution | Carried by |
|---|---|---|
| 1 | **RESOLVED — leave `tests/` as-is.** V-005's strip and V-009's Rule 7 are scoped to `scripts/`, `references/`, `skills/`, `eval/`. | Steps 6, 11 |
| 2 | **RESOLVED — keep `REQ-*` IDs.** `REQ-` is excluded from Rule 7's pattern. | Steps 6, 11 |
| 3 | **RESOLVED — option (a).** Tighten the backlog AC template to the artifact-level assertion alone, plus a one-time `find_spec("pytest")` guard. `validate.sh`'s SKIP branch is left non-fatal. | Step 9 |
| 4 | **RESOLVED — option (a) only.** Narrow item 018's AC to `skills/forge-verify/` and `skills/forge-fix/`. No `state-ecr` follow-up filed. | Step 9 |
| 5 | **RESOLVED — option (a).** Document the `auto-verify-pending` CLI exposure in `references/shared-conventions.md`; the enum is left derived, not carved out. | Step 9 |
| 6 | **RESOLVED — taken, option (b).** Add the one-line note under the stamp fence; the `--host claude` literal itself is left untouched. | Step 13 |
| 7 | **RESOLVED — deferred.** Step 12 (smoke command) is not applied; `smokeCommand` stays `null` and V-018 remains open as advisory. | — (Step 12 skipped) |
...
Applied 2026-07-31 by `/feature-forge:forge-fix stage-exit-coverage --served-stage forge-5-loop`.

- Step 1: [APPLIED] 2026-07-31 — Deleted `tests/fixtures/minimal-canon/**/__pycache__/` (24 `.pyc` files across `scripts/` and all six `expected-adapters/*/scripts/`). Made `hash_tree` skip `__pycache__` parts and `.pyc` suffixes, with a docstring explaining the stateful-hash failure mode. Added `test_hash_tree_ignores_bytecode_caches`, which plants debris under a **copied** fixture and asserts both the self-comparison and the `expected-adapters` comparison stay green. V-001 reproduced before the fix and passes after.
- Step 2: [APPLIED] 2026-07-31 — Added `state-verify --status skipped` fences to `skills/forge-5-loop/SKILL.md` Step 5b and `skills/forge-4-backlog/SKILL.md` Step 6 (after the `state-complete`/`state-note` block), each with the portable-root prelude, the `--epic` sentence inside the 12/8-line window, and the exit-2 rule. forge-5-loop keeps its "(mirrors `forge-4-backlog`'s skip handling)" cross-reference. Call-site total 32 → 34; `MIN_CALL_SITES` raised to match.
- Step 3: [APPLIED] 2026-07-31 — Added Guard 3 to `tests/test_state_verb_call_sites.py`: every surface in `SKIP_RECORDING_SURFACES` must ship a `state-verify … --status skipped` invocation, plus `test_the_skip_guard_is_not_vacuous`. Mutation-tested: flipping forge-5-loop's fence to `--status passed` fails the guard as intended.
- Step 4: [APPLIED] 2026-07-31 — Added `tests/test_capability_determination_prose.py` (9 tests). Roster derived from `CANONICAL_EXIT_SITES` filtered by the capability lead-in, yielding exactly the six determining surfaces (floor 6). Three clauses asserted with per-clause alternative phrasings, since `forge-verify` states clause (b)/(c) in wording the authoring stages do not share. Three spec-mandated negative controls operate on mutated copies via `_assert_capability_prose`, plus `test_this_guard_is_not_skippable`.
- Step 5: [APPLIED] 2026-07-31 — Added `test_an_advertised_but_unavailable_dispatch_never_promotes_the_deferred_command` to `tests/test_auto_verify.py`, immediately after the interrupted-dispatch test. Asserts (b) `primaryCommand` is the verify command under `--verify-capability manual` and (c) the first payload's `deferredCommand` is never promoted, including that it stays out of the fenced block in `nextSteps`. **Deviation:** V-004 expected `verifyGate == "manual-print"`; the contract emits `"none"` on the `runInStageVerify: true` path, so the test pins `"none"` and records why. Asserting `manual-print` would have pinned a bug.
- Step 6: [APPLIED] 2026-07-31 — Stripped every spec coordinate from `scripts/forge-session.py`, `scripts/epic-manifest.py`, and `eval/run-compliance-eval.py` (0 remaining in all three). 192 parentheticals removed mechanically, 53 prose-woven citations rewritten by hand. `REQ-*` identifiers preserved (Decision 2) and `tests/` untouched (Decision 1). All three load-bearing cases handled: `state_verify`'s docstring now states the metadata matrix itself as a five-bullet list verified against the validation code, with the "the matrix is authoritative, not this docstring" clause deleted; `07-testing-strategy.md §3 asserts the literal` → `tests/test_stage_exit.py`; both `tech-spec §3.5 forbids` sites restated directly. `scripts/forge-bootstrap.py` left untouched per the step's instruction. Verified: all files parse, `ruff check scripts/ eval/` clean, mirrored JSON-loader block unaffected (it carried zero citations).
- Step 7: [APPLIED] 2026-07-31 — Replaced the `stage-exit` synopsis in the module docstring with the corrected four-line form covering `--owner`, `--outcome`, `--verify-mode`, `--served-stage`, `--verify-capability`, and `--host claude|generic|pi`. Added `test_the_stage_exit_synopsis_lists_every_flag_the_parser_accepts` to `tests/test_stage_constants_parity.py`, deriving all 13 `p_exit` long options from the registration; verified against `python3 scripts/forge-session.py --help`.
- Step 8: [APPLIED] 2026-07-31 — V-007: reworded `README.md` and `references/forge-config-schema.json` to the shipped ignored-with-a-warning semantics (no code change). V-010: replaced "authoritative state is unknown" with "the verify entry is unchanged" in `skills/forge-verify/SKILL.md` and `skills/forge-fix/SKILL.md`, and added the `stage-exit` pre-payload-debt paragraph to `references/stage-exit-protocol.md` — placed **after** the `scripted-stage-exit-stamp` END marker, since the stamped region is compared byte-for-byte across all nine skill copies. V-008: corrected the test name to `test_the_contract_comparison_ignores_prose_but_not_structure` and the constant to `SCHEMA_CONTRACT_OUTSIDE_VERIFY_ENTRY_SHA256` in `07-testing-strategy.md` §4.4. V-013: added `test_doctor.py` and `test_rank_features.py` rows to `01-architecture-layout.md` §2.
- Step 9: [APPLIED] 2026-07-31 — V-012(a): tightened all 30 occurrences of the backlog AC template to the artifact-level assertion alone, and added `tests/test_gate_pytest_reachability.py` asserting `find_spec("pytest") is not None` plus that `validate.sh`'s SKIP branch still increments `WARNINGS` so it stays distinguishable from a clean run. V-014(a): narrowed item 018's AC to `skills/forge-verify/` and `skills/forge-fix/`. V-015(a): documented in `references/shared-conventions.md` that `auto-verify-pending` is written only by `stage-exit`'s scheduling boundary and is exposed for repair/inspection. No item `status` or `completedAt` touched — all 32 remain `done` (asserted programmatically).
- Step 13: [APPLIED] 2026-07-31 — V-016 option (b): added the one-line note under the stamp fence in `references/stage-exit-protocol.md`, outside the BEGIN/END markers, explaining the `--host claude` literal and why it is not a placeholder. The fenced line itself is unchanged; `tests/test_stage_exit_protocol.py` and `tests/test_adapter_host_neutrality.py` both pass.
- Step 12: [SKIPPED] 2026-07-31 — Deferred by explicit user decision (Decision 7). `smokeCommand` stays `null`; V-018 remains open as advisory.
- Step 10: [APPLIED] 2026-07-31 — `python3 scripts/build-adapters.py` regenerated all six mirrors in lockstep (31 files each); `--check` exits 0 (no drift). `bash scripts/validate.sh` run **twice in succession**: both exit 0, both print `PASS: epic-manifest pytest suite` (not `SKIP`), `✔ Validation passed`, and `ruff check scripts/ eval/` → `All checks passed!`. Suite 1746 → **1762 passed, 2 skipped** (+16, within the plan's expected +12–16). V-001's idempotency defect is confirmed fixed by the second green run.
  - **In-flight correction:** Step 2's fence pushed `skills/forge-5-loop/SKILL.md` to 310 body lines against the hard 300-line cap (REQ-CAP-01), failing `test_skill_body_is_within_the_line_cap`. The fence is required at that site, so the lines were reclaimed inside the same file: the Step 5b prose was tightened and four hard-wrapped paragraphs (the intro, the runner note, the token-substitution rider, and §3f) were rewrapped to the long-line style used by the rest of the file. No normative content removed. Body now 295 lines / 4644 words, both under cap.
- Step 11: [NOT APPLIED] 2026-07-31 — **Precondition not satisfiable within this fix's scope.** V-009 scopes Rule 7 to `scripts/`, `references/`, `skills/`, and `eval/` with a three-file allowlist, and states "land this only after V-005's cleanup, or it fails immediately." After Step 6 the three files V-005 named are clean, but **26 further files in that scope still carry 259 citations** (`scripts/` 162, `references/` 45, `skills/` 52, `eval/` 1) — `build-adapters.py` (80), `forge-bootstrap.py` (49, explicitly excluded by Step 6), `check-spec-purity.py` (21), `references/loop-agent-selection.py` (18), `references/stacks/*.md`, and others, most citing *other features'* spec sets. Landing Rule 7 would either red-gate the build or require a 26-entry allowlist that voids the rule for its largest offenders. Purging them is a repo-wide refactor this fix pass was not scoped to make, so the rule is left unlanded and reported to the user for a scoping decision.
- Step 12: [NOT APPLIED] — see the skip note above (Decision 7).

### Findings not addressed by this plan

- **V-011** (call-site guard's window bounds unpinned; failure message describes only the lookbehind half) — no execution step was written for it. `MIN_CALL_SITES` was updated 32 → 34 as Step 2 required, but `LOOKBEHIND`/`LOOKAHEAD` remain unpinned and the message still says "within {LOOKBEHIND} lines above".
- **V-017** (4 substantive ruff violations plus 18 E501s in `tests/`) — no execution step. `ruff check scripts/ eval/`, the configured scope, is clean; `ruff check tests/` still reports 21 errors.
- **V-018** — deferred by Decision 7.
