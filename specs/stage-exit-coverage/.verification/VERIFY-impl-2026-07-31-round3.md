...
- Step 1: [APPLIED] 2026-07-31 — Replaced `forge-fix` SKILL.md:110's production-stage clause-(c) sentence with the branch-appropriate "presented through the Step 6 gate and dispatched on the affirmative choice" wording; regenerated all six adapter mirrors (`build-adapters.py --check` exit 0). Mutation-verified: deleting the new sentence turns `test_deleting_the_auto_path_through_the_gate_fails_the_guard[skills/forge-fix/SKILL.md]` RED.
- Step 2: [APPLIED] 2026-07-31 — `test_the_failure_message_describes_the_whole_window` now reads `inspect.getsource(test_every_state_verb_call_site_carries_the_epic_instruction)` instead of its own file (`import inspect` added). Mutation-verified: reverting Guard 1's message to lookbehind-only wording now turns it RED (it stayed green before).
- Step 3: [APPLIED] 2026-07-31 — `LOOKAHEAD` 8 → 3; pin is now `assert LOOKAHEAD <= CALL_SPAN` with a buried-mandate message. Constant comment and pin docstring state both measured maxima explicitly (10 above, 1 below). Guard 1 stays green across all 34 sites at the narrower window.
- Step 4: [APPLIED] 2026-07-31 — Clause (c) split into **three** independently-required sub-clauses, not two. `"choice 2 omitted"` dropped entirely. Two deviations from the finding's literal prescription, both forced by measurement: (i) c1 needs a third phrasing, `"reuse the Standard Verify Gate block for consent"` — the four authoring stages state the gate half only that way, so the finding's two-fragment c1 would have red-gated canon with no defect present; (ii) the finding's single c2 (no-skip + no-advance in one any-of list) does **not** catch its own mutation (a) — inverting `"is never grounds to skip verification"` leaves `"never grounds to fence the production successor"` matching, measured GREEN on four surfaces. Split into c2 (no-skip) and c3 (no-advance). Negative control 3 split into 3a/3b/3c, each parametrized over all 6 surfaces. Mutation-verified RED: (a) skip-inversion on all 4 authoring stages; (b) lead-in deletion keeping the mechanism tail, all 4; (c) `forge-verify` disjunction rewrite; (c2) same rewrite on `forge-fix`.
- Step 5: [APPLIED] 2026-07-31 — `test_the_controls_cover_every_determining_surface`'s tautological `SURFACE_IDS == [...]` assertion replaced with the source-text derivation check from V-007, plus a comment recording why the old form could not fail. Mutation-verified: replacing only the module-level `ALL_SURFACES = _capability_surfaces()` with a hardcoded literal goes RED.
- Step 6: [APPLIED] 2026-07-31 — `_SPEC_CITATION_RE` branch 3 widened to `\btech-spec(?:\.md)?\s*§`, closing V-003's material miss. **The finding's second branch, `\btech-spec\.md\b` (bare filename), was deliberately NOT added** — measured, it turns five clean shipped surfaces red (`references/process-overview.md`, `skills/forge-2-tech`, `forge-6-docs`, `forge-guide`, `forge`), every hit a legitimate reference to the *pipeline artifact* `tech-spec.md` that forge-2-tech writes and the navigator displays. The only way to keep the gate green would be grandfathering five files to make new violations pass, which the list's own contract forbids. Unlike `03-verification-state.md`, `tech-spec.md` is an output filename as well as a spec name; a citation is a filename plus a coordinate. Rationale recorded in the pattern's docstring. Added the `"the tech-spec.md §3.4 rules govern this"` parametrize case; mutation-verified RED on reverting the regex. `check-spec-purity.py` exit 0, no grandfather entry added.
- Step 7: [APPLIED] 2026-07-31 — Dropped the dangling `tech-spec.md §3.4` citation from the module docstring (:4) and the `Rule` enum docstring, pointing at the enum's per-rule provenance comments instead. Re-derived **all 29** grandfather annotations against the final regex (not just the one V-006 named): 28 exact, `scripts/check-spec-purity.py` corrected 21 → **25**. Added the "shrinking only" enforcement V-006 called optional — the `# N` annotation is now a ceiling, and every entry must carry one. Mutation-verified RED three ways: adding a citation to a grandfathered file, lowering an annotation below its live count, and stripping an annotation.
- Step 8: [APPLIED] 2026-07-31 — Re-gated. `build-adapters.py` then `--check` exit 0 (no drift). `bash scripts/validate.sh` twice back-to-back: exit **0** both, `All checks passed!` both, **1802 passed / 2 skipped** both (225.77s, 214.30s) — up 13 from 1789, exactly the 12 new capability sub-clause controls plus the 1 new citation-form case. `tests/fixtures` bytecode **0** after each run. `ruff check scripts/ eval/` exit 0; `python3 scripts/check-spec-purity.py` exit 0; `ruff check tests/ --select F841,F541` clean with the same **19** cosmetic hits as before this pass (decision 2 / option (b) preserved). Every guard touched in Steps 2–5 was accepted on a **mutation going red**, not on the suite staying green — 16 mutations run in total across Steps 1–7, all detected.

### Deviations from the findings' prescribed fixes

Two, both forced by measurement and both narrowing scope rather than widening it:

1. **V-003, Step 6** — the second regex branch `\btech-spec\.md\b` was not applied. It flags the pipeline artifact filename, not a citation, and turns five clean shipped surfaces red; landing it would have required grandfathering five files to make new violations pass. The coordinate branch that V-003's argument actually rests on (`tech-spec.md §N`) is applied and mutation-verified.
2. **V-004, Step 4** — clause (c) was split into **three** sub-clauses, not two, and c1 carries a third accepted phrasing. The findings' own c1 list would have red-gated four compliant authoring stages, and its single merged c2 does not catch its own mutation (a). Both were measured before deviating; the stricter split catches all three mutations the finding listed.
