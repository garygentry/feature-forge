...
> **All four decisions were resolved by the user on 2026-07-30, each taking the
> recommended option. They are RESOLVED — a fix agent applies the recommended path in
> every case and does not re-ask.**
>
> - **D1 → Specify the edits.** Add `04` §4.3 registering `state-verify` and defining the
>   named `state-note` recipe; keep both layout rows.
> - **D2 → In scope, convert it.** Add `findings-template.md` to both layouts and `04` §1,
>   and add `04` §5.4 replacing the heredoc snippet with the `state-verify` call.
> - **D3 → `warnings: list[str]`,** ordered epic-fallback → debt-metadata →
>   revision-mismatch.
> - **D4 → Swallow the `OSError`** (`try/except OSError: pass` around
>   `warn_duplicate_keys`), plus the stderr-unwritable row in `05` §3.3.
>
> The two non-blocking defaults also stand: **V-007 → derive** the `00` §2 constants via
> `get_args`; **V-026 → regex + `ast.literal_eval`** per the drift-guard convention.
...
- Step 1: [APPLIED] 2026-07-30 — `00-core-definitions.md`: §3-not-§5 `stage_exit` citation (V-001); `EpicReconcile.deferred` rewritten as a command (V-002); `autoVerifyDebtRecorded` combination marked unreachable (V-003); `stageNoun: str` added (V-004); `verifyStage: str | None` added (V-005); `warning: str` → `warnings: list[str]` with fixed order, per D3 (V-006); `EXIT_STAGES`/`EXIT_OUTCOMES` derived via `get_args` plus the `VERIFY_MODE_TO_STAGE` parity sentence (V-007); `cmd_state_verify` docstring given full `Args:`/`Returns:` (V-008); `invalidAutoVerifyKeys` render template added (V-011, `00` half). All 11 python blocks parse.
- Step 2: [APPLIED] 2026-07-30 — `02-stage-exit-routing.md`: §5.2 deferred-line precedence rule between `epicReconcile["deferred"]` and `deferred_command` (V-002); §8 sibling-path resolution + `sys.executable` + `timeout=10` with `TimeoutExpired`/`OSError` → `UsageError`, plus the §10 table row (V-009); §5.1 "Consent variant on a `none` gate" with choice 2 omitted, mirrored into `04` §3.3 step 2 and anchored in tech-spec §3.3 (V-010); §9 exact epic-member fallback warning template (V-011); `03-verification-state.md` added to Dependencies (V-012). `04` §3.3 step 1 updated for the `warnings` list. No bare `warning` field references remain in any spec.
- Step 3: [APPLIED] 2026-07-30 — `03-verification-state.md`: new §5.4 "Downstream pre-flight parity (REQ-DEBT-05)" naming both live call sites (V-013); §5.3 now names `verifyState`/`verifyStage`/`verifyCommand` as the emitted JSON keys (V-005); §3.3 `findings-reported` row and §7.1 fail-closed list given relative-and-contained validation for `--findings-file` (V-014). `04-skill-integration.md`: §7.2 extended for the docs backstop enumeration, new §6.4 for the loop Step 1b gate (V-013), new §6.5 for REQ-FOLLOW-01 (V-021), coverage table rows added.
- Step 4: [APPLIED] 2026-07-30 — Unowned canon files given owners. `01` §2 and tech-spec §2.1: added `skills/forge/SKILL.md` and `skills/forge-verify/references/findings-template.md`, re-annotated `shared-conventions.md`, annotated the two pre-flight skills. `04`: new §4.3 (shared-conventions `state-verify` registration + `state-note` recipe, per D1; navigator read-side and nested dispatch) (V-015, V-019); §5.1 given the ownership-token carrier and the `state-verify` fence with the adjacent `--epic` sentence (V-018, V-020); §5.2/§5.3 given the falsified-canon deletions — R4 exclusion blockquote, forge-fix Step 5 write, the two "fresh in the ledger" claims, the skip-re-verify wording (V-017); new §5.4 converting findings-template.md, per D2 (V-016); §1 scope list and §10 regeneration batch updated.
- Step 5: [APPLIED] 2026-07-30 — `05-config-and-distribution.md`: `_load_config` guards `warn_duplicate_keys` with `except OSError: pass` per D4, with the rationale and the bootstrap split stated (V-023); §3.3 matrix row added; §5.2 fixture path corrected to `tests/fixtures/minimal-canon/expected-adapters/…` (V-024); REQ-CONFIG-04 coverage row `§5.1` → `§3.3` (V-022, `05` half). `06-compliance-and-coverage.md`: `score_branch_path` raises `RuntimeError` not `KeyError`, with explicit pre-indexing validation (V-025); §2.1 given the `EXIT_STAGES` access convention and the no-hardcoded-list prohibition (V-026).
- Step 6: [APPLIED] 2026-07-30 — `07-testing-strategy.md`: §2.3 minimal-canon stubs described accurately and loader evidence retargeted to committed `adapters/` (V-027); §6.3 byte-equality scoped to function bodies with the Pi substitution carved out (V-028); §3.4 given the `none` row, the consent-barred row, the `CLEAN_ROOM_UNAVAILABLE` row, the render-status timeout case, and verbatim warning literals (V-029, V-030, V-031, V-009, V-011); §6.2 given the clause-(b) capability assertions with copied-string negatives, the `test_state_verb_call_sites.py` bullet, the no-`os.replace`-under-`skills/` assertion, the pre-flight and navigator bullets, and the shared-conventions bullet (V-029, V-018, V-016, V-013, V-019, V-015); §6.1 given the `EXIT_STAGES` access convention (V-026); §4.1 given `verifyStage`, legacy `pending`, and pre-flight parity rows (V-005, V-031, V-013); §4.2 given `--findings-file` rejections (V-014); §4.4 reworded to re-pin-plus-structural-diff preserving the negative control (V-033); §5.3 given the D4 expectation split (V-023); §8.1's two overstated claims qualified (V-031, V-029); §8.2 and the Verification checklist given the non-fatal-pytest qualifier (V-032); coverage table rows added.
- Step 7: [APPLIED] 2026-07-30 — `TRACEABILITY.md`: REQ-EXIT-03 → §3, §5–§6; REQ-EXIT-04 → §3.3, §6 + the `04` §5.1 carrier; REQ-EXIT-06 → §4–§5; REQ-CONFIG-04 → `05` §2.1, §3.3 (V-022); new anchors added for REQ-FOLLOW-01 (`04` §6.5), REQ-FOLLOW-02 (`04` §4.2/§4.3), REQ-DEBT-05 (`03` §5.4 + `04` §4.3/§6.4/§7.2), REQ-STATE-03 (`04` §5.4). Gates re-run: traceability 55 REQs / 0 uncovered / 0 orphaned / valid; `rauf backlog validate` valid; 57 python blocks across all specs parse. One self-inflicted regression caught and fixed by the gate — a `REQ-GEN-05` citation borrowed from another feature's PRD registered as an orphaned reference; the invariant is now named in prose instead.
