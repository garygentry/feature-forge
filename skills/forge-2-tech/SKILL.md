...
1. Update `{specsDir}/{feature}/.pipeline-state.json`:
   ...
   - Check downstream stages (forge-3-specs, forge-4-backlog, forge-5-ralph-loop, forge-6-docs). If any have `basedOnVersions` referencing an older version of forge-2-tech, set their status to `stale`
