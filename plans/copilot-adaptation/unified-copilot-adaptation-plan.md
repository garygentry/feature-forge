...
| C. Rauf Native Operator Adapter | G1 closed; rauf provider contract stable for child/supervisor separation | Generated operator plugin, two agent boundaries, operator dependency/prose reconciliation, installed child instructions, drift/version/package gates (`RAUF-201`–`RAUF-204`, including `RAUF-202R`) | All five task rows complete; four skills/two agents discovered with tested skill dependencies; behavioral tool denials pass; child instructions are ownership-safe; generation is idempotent; `pnpm gate` passes | E |
...
Status: Complete. `RAUF-101` through `RAUF-108` and the rauf gate passed on 2026-08-23.
...
- [x] **RAUF-105 — Classify Copilot failures without Claude semantics**
  Repo: rauf. Depends on: RAUF-103.
  ...
  Evidence (2026-08-23): a Copilot-owned classifier maps auth, invalid-model, permission, limit/
  credit, and other process failures to the existing pending/circuit-breaker infrastructure path;
  timeout to the existing blocked timeout path; and malformed or missing completion output to the
  existing retry/defer path. Runner cancellation takes precedence and remains the existing loop-
  cancel outcome; spawn errors remain the existing fatal execute-error path. The provider exposes
...
- [x] **RAUF-107 — Add mock and fixture runtime matrix**
  ...
  Evidence (2026-08-23): a table-driven provider matrix covers Copilot signals, malformed/unknown
  JSONL, nonzero infrastructure exit, invalid model, auth, in-band permission denial, timeout, and
  prompt cleanup. Real detached Node process groups prove timeout/abort descendant cleanup. Runner
  coverage proves work and review use reconstructed provider text; this exposed and fixed review's
  raw-JSONL parsing bug. CLI tests prove detached `copilot` forwarding and resume preservation.
  The sandbox now emits Copilot JSONL instead of plain text and adds sanitized no-signal,
  malformed/unknown, auth, model, and permission scenarios. The full sandbox passed 192 assertions;
  161 loop and 100 CLI focused tests, both typechecks, lint, formatting, shell syntax, and fixture
  parsing passed. Milestone: rauf commit `9bbc3e5`.

- [x] **RAUF-108 — Make install/config/UI provider-aware**
  ...
  Evidence (2026-08-23): install/init validate registry IDs, persist `provider: "copilot"`, and
  preserve it on reinstall while omitted flags retain existing precedence. Core preflight checks
  only the selected binary; dedicated Copilot marker config rejects `providerConfig`, preventing
  arbitrary argv forwarding. Direct, detached, CLI resume/review, strict web start/review/resume,
  and compiled CLI paths preserve provider and `--no-model` policy. Agent discovery now reports
  binary presence separately from tri-state authenticated readiness; Copilot auth is explicitly
  unknown because CLI 1.0.78 has no safe non-mutating auth probe. Compiled install/reinstall/init/
  agents smokes passed. The full rauf gate passed 2,271 package tests and 83 repository-script
  tests plus build, schema/version/adapter drift, typecheck, lint, formatting, and docs checks.
  Milestone: signed rauf commit `5f3710b`.

### Cross-phase durability checkpoint

- [x] **TRACK-001 — Make planning and runtime evidence fresh-clone durable**
  Repo: feature-forge/shared. Depends on: RAUF-202.
  Track and push every authoritative source plan and sanitized evidence file referenced by this
  plan. Add a `RAUF-202` receipt containing the exact platform/version/commands/prompts/results and
  cleanup state, reconcile stale cursor and plugin-format claims, and verify a fresh clone can read
  the runbook, execution ledger, source plans, and evidence without ignored local files. The
  operational acceptance and current state live in `EXECUTION.md`.
  Evidence (2026-08-25): local session history retained only the 2026-08-24 summary, so the minimum
  disposable reviewer/driver probes were rerun against rauf `02f8e67` with exact Copilot CLI 1.0.80
  through `npx`. The tracked receipt records exact sanitized prompts/argv, stable hashes, dedicated
  edit-tool denial, two schema-v1 status polls, hostile implementation/signal refusal, cleanup, and
  clean registry/worktree state. All adaptation plans/evidence are visible to Git, the milestone is
  pushed on `docs/copilot-g2-contract`, and fresh-clone readability passed.
...
- [x] **RAUF-201 — Generate native Copilot operator bundle**
  ...
  Evidence (2026-08-24): `scripts/build-copilot-bundle.ts` generates the nine-file
  `adapters/copilot/` Agent Plugins 1.0 bundle from all four canonical skills and both canonical
  agents. Canonical frontmatter and agent policy mappings fail loud, source provenance is embedded,
  output ordering is deterministic, and `findDrift` detects missing, changed, and stale files.
  `copilot:generate`/`copilot:check` are available; the drift check is intentionally not wired into
  `pnpm gate` until `RAUF-204`. Six focused tests, ESLint, Prettier, generation, and the no-op drift
  check passed with Bun 1.3.10. The full rauf gate also passed 2,277 package tests and 89 script
...
- [x] **RAUF-202R — Reconcile operator dependency, prose, and evidence residuals**
  Repo: rauf. Depends on: RAUF-202 and TRACK-001.
  Resolve each operator agent's dependency on `review-backlog` or `drive-rauf-loop` through tested
  Copilot behavior rather than an aspirational body reference. Replace portable canonical `Task
  tool` wording with host-neutral delegation prose, retain host-specific specialization only in
  host output, and record the accepted plugin-only repository topology from D7. Regenerate, run the
  focused generator/runtime checks, and preserve a sanitized receipt.
  Evidence (2026-08-25): rauf commit `4668553` gives both Copilot operator policies fail-loud
  `requiredSkill` mappings and composes the complete canonical skill bodies into generated agent
  context with dual-source provenance/reporting. Copilot CLI 1.0.80 returned reviewer rules and
  driver thresholds found only in the composed contracts, with no tool calls or file edits.
  Portable `Task tool` wording is host-neutral; Codex/Copilot/Pi outputs and drift checks pass; D7
  remains plugin-only. The full isolated-home gate passed 2,188 package tests plus 91 script tests.
...
- [x] **RAUF-203 — Preserve installed child instructions**
  ...
  iteration provider retains `--no-custom-instructions` and receives `.rauf/RAUF.md` through the
...
- [x] **FORGE-102 — Emit native Copilot custom agents**
