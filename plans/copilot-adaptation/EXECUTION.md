...
| Rauf adapter gates/package proof | `RAUF-204`; rauf commit `8d65441`; exact drift/version/binary/npm receipt in `evidence/rauf-204-adapter-gates-packaging-2026-08-25.md` |
| Feature-forge native foundation | feature-forge commits `7754a3b`, `74f269b`, and `fdc26ff`; native skills/agents, runtime-proven legacy Copilot manifest, distribution-aware invocation, composed verifier contract, memory-limit translation, and exact-name dispatch boundaries; `FORGE-101`–`FORGE-102` complete |
...
| 4 | `RAUF-204` | rauf | DONE | `RAUF-203` | Pushed rauf commit `8d65441` makes Copilot drift/distribution release-blocking, version sync includes the generated plugin, negative guards fail correctly, a compiled binary enumerates `copilot` and installs embedded instructions, and npm dry-run is the exact four-file launcher. Full gate: 2,194 package + 91 script tests. Receipt: `evidence/rauf-204-adapter-gates-packaging-2026-08-25.md`. |
| 5 | `FORGE-101` | feature-forge | DONE | `TRACK-001` | Pushed commit `74f269b` implements distribution-neutral `invoke-skill:` notation with explicit plugin/direct overlay forms, retains and accurately classifies the five-field legacy Copilot manifest, runtime-proves 13 installed skills and the overlay contract on CLI 1.0.80, and passes the clean-shell full gate. Receipt: `evidence/forge-101-native-skills-invocation-2026-08-25.md`. |
| 6 | `FORGE-102` | feature-forge | DONE | `FORGE-101` | Pushed commit `fdc26ff` composes canonical `forge-verify` fail-loud into the verifier, translates unsupported memory promises, and passes exact-name parallel CLI 1.0.80 dispatch with intended edit boundaries plus the clean-shell full gate. Fresh-clone durability passed. Evidence: `evidence/forge-102-native-agent-dispatch-2026-08-25.md`. |
| 7 | `FORGE-103` | feature-forge | DONE | `FORGE-101` | Pushed commit `7cfdb31` implements override/self-location/Copilot managed-personal-project precedence without generic `PLUGIN_ROOT`; Copilot CLI 1.0.80 runtime probes, complete/degraded and adversarial-path coverage, 815 focused tests, cleanup, full gate, and fresh-clone regeneration/hash checks pass. Evidence: `evidence/forge-103-runtime-root-resolution-2026-08-26.md`. |
| 8 | `FORGE-104` | feature-forge | DONE | `FORGE-101` | Pushed commit `c65b691` adds recursive/flat Copilot mirrors, trusted lexical and symlink-ancestor containment, dry-run reports, copy/symlink/Windows behavior, proven-only ownership, exact uninstall, and generic-`.github` negative detection. Full gate and clean fresh-clone installer suite pass. Evidence: `evidence/forge-104-installer-placement-primitives-2026-08-26.md`. |
| 9 | `FORGE-105` | feature-forge | DONE | `FORGE-102`, `FORGE-103`, `FORGE-104` | Pushed commit `797fc60` installs one complete runtime at project `.github/feature-forge` or personal `~/.copilot/feature-forge`, runtime-proves native skill/agent discovery and helper execution on CLI 1.0.80, reports `verified-current`, and validates trusted primary ownership before uninstall. Post-fix full gate and a clean fresh clone pass 203 installer tests. Evidence: `evidence/forge-105-fresh-project-personal-installs-2026-08-26.md`. |
| 10 | `FORGE-106` | feature-forge | ACTIVE | `FORGE-105` | Local implementation passes trusted current/legacy identity validation, phased apply/verify/cleanup/write-last migration, managed-region conflict/force behavior, old personal copy/symlink roots, failure recovery, dry-run parity, repeated update, exact uninstall, independent review, and the full gate (2,496 Python + 208 installer + 11 Pi-source tests). Receipt: `evidence/forge-106-fail-safe-legacy-migration-2026-08-27.md`. Await authorized commit/push and fresh-clone durability before DONE. |
...
1. The current feature-forge `adapters/copilot/plugin.json` is accepted by Copilot but has no Agent
   Plugins 1.0 `$schema`; FORGE-101 deliberately retains and names it the legacy Copilot plugin
   format. A future 1.0 migration must reopen the format claim and prove the new namespace/layout.
...
| 2026-08-25 | `RAUF-204` | Wired Copilot drift/distribution into `pnpm gate`, version-locked the generated plugin, negative-tested drift/version failures, verified a release-shaped compiled binary and exact npm launcher dry-run, and passed the full clean-home gate. All disposable artifacts were removed; authorized commit `8d65441` is pushed. | feature-forge tracker update on `docs/copilot-g2-contract`; rauf `8d65441` | `RAUF-204` done; Phase C exits; continue `FORGE-101`. |
| 2026-08-25 | `FORGE-101` | Implemented distribution-neutral Copilot invocation notation across descriptions, bodies, references, and scripted exits; retained and accurately classified the five-field legacy Copilot manifest; installed/runtime-proved 13 skills and the plugin/direct overlay contract on CLI 1.0.80; passed the clean-shell full gate (2,481 Python, 182 installer, 11 Pi source tests). Disposable plugin/marketplace/temp state was removed. Authorized commit `74f269b` is pushed. | feature-forge `74f269b`; rauf unchanged at `8d65441` | `FORGE-101` done; continue exactly one cursor at `FORGE-102`. |
| 2026-08-25 | `FORGE-102` | Pushed fail-loud canonical verifier-skill composition and Copilot memory-limit translation; Copilot CLI 1.0.80 dispatched all three exact names in parallel, enforced researcher/verifier denial and writer success, and exposed the composed CHECK-I21/I22 marker. The clean-shell full gate passed (2,484 Python, 182 installer, 11 Pi source tests); fresh-clone evidence/hash checks passed. | feature-forge `fdc26ff`; rauf unchanged at `8d65441` | `FORGE-102` done; continue exactly one cursor at `FORGE-103`. |
| 2026-08-26 | `FORGE-103` | Pushed runtime-root implementation with neutral override precedence, Copilot managed/personal and nested-project roots, project-over-global bootstrap ordering, guarded ancestor traversal, adversarial-path coverage, and preserved degraded handling. Copilot CLI 1.0.80 passed all four runtime layouts; 815 focused tests, cleanup, drift/full gates, and fresh-clone regeneration/hash checks pass. | feature-forge `7cfdb31`; rauf unchanged at `8d65441` | `FORGE-103` done; advance exactly one cursor to `FORGE-104`. Evidence: `evidence/forge-103-runtime-root-resolution-2026-08-26.md`. |
| 2026-08-26 | `FORGE-104` | Pushed recursive native skills, flat Copilot agents, trusted lexical/symlink containment, proven ownership, dry-run placement reporting, exact lifecycle, Windows fallback, and conservative detection. Independent-review findings were fixed; clean-shell full gate passes 2,496 Python (2 skipped), 201 installer, and 11 Pi-source tests. A disposable clone at the pushed commit repeated all 201 installer tests and remained clean. | feature-forge `c65b691`; rauf unchanged | `FORGE-104` done; advance exactly one cursor to `FORGE-105`. Evidence: `evidence/forge-104-installer-placement-primitives-2026-08-26.md`. |
| 2026-08-26 | `FORGE-105` | Pushed implementation moves only fresh personal primary installs from `~/.github/feature-forge` to `~/.copilot/feature-forge`, retains project `.github/feature-forge`, mirrors native skills/agents, and raises confidence against current Agent Skills docs. Copilot CLI 1.0.80 discovered/invoked project and personal skills/agents, resolved each complete root, and ran the installed doctor helper. Review's forged-primary finding is fixed with exact trusted primary-ownership validation. The post-fix clean-shell full gate passes 2,496 Python (2 skipped), 203 installer, and 11 Pi-source tests; a disposable clone of the pushed exact commit repeats all 203 installer tests and stays clean. | feature-forge `797fc60`; rauf unchanged | `FORGE-105` done; advance exactly one cursor to `FORGE-106`. Evidence: `evidence/forge-105-fresh-project-personal-installs-2026-08-26.md`. |
| 2026-08-27 | `FORGE-106` | Uncommitted implementation removes fresh managed-block creation, validates historical personal ownership, applies and verifies native files before old cleanup, safely reconciles managed-region conflicts, migrates copy/symlink roots, and proves write-last recovery, dry-run parity, repeated update, and exact uninstall. Independent re-review reports no blocker; the full clean-shell gate passes 2,496 Python (2 skipped), 208 installer, and 11 Pi-source tests. | feature-forge dirty tree over `04eff05`; rauf unchanged | Keep `FORGE-106` ACTIVE pending explicit commit/push authorization and fresh-clone durability. Evidence: `evidence/forge-106-fail-safe-legacy-migration-2026-08-27.md`. |
