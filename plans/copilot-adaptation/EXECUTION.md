...
| 9 | `FORGE-105` | feature-forge | ACTIVE | `FORGE-102`, `FORGE-103`, `FORGE-104` | Dirty implementation and Copilot CLI 1.0.80 proof pass: fresh project/personal installs expose native skills and agents plus one complete runtime at `.github/feature-forge` or `~/.copilot/feature-forge`; both roots execute the installed doctor helper and confidence is `verified-current`. Review's forged-primary finding is fixed, final targeted review is clear, and the post-fix clean-shell full gate passes 2,496 Python (2 skipped), 203 installer, and 11 Pi-source tests. Await authorized commit/push and fresh-clone durability before DONE. |
...
| 2026-08-26 | `FORGE-105` | Dirty implementation moves only fresh personal primary installs from `~/.github/feature-forge` to `~/.copilot/feature-forge`, retains project `.github/feature-forge`, mirrors native skills/agents, and raises confidence against current Agent Skills docs. Copilot CLI 1.0.80 discovered and invoked project/personal probe skills, resolved each complete root, ran the installed doctor helper, and dispatched `forge-researcher` in both scopes; cleanup passed. Review's forged-primary finding is fixed with exact trusted primary-ownership validation; the post-fix clean-shell full gate passes 2,496 Python (2 skipped), 203 installer, and 11 Pi-source tests. | feature-forge dirty tree on `5a401ca`; rauf unchanged | Keep `FORGE-105` ACTIVE pending authorized commit/push and fresh-clone durability. Evidence: `evidence/forge-105-fresh-project-personal-installs-2026-08-26.md`. |
