...
| `loopRunner`          | object  | rauf defaults           | Loop-runner binding for `forge-5-loop` (`bin`, command templates, `defaultAgent`, `minRunnerVersion` — floor **rauf ≥ 0.6.0**). Opt-in zero-prompt launch: `reviewMode` (`prompt`\|`always`\|`never`) gates the Run-mode question, `agentMode` (`prompt`\|`auto`) gates the agent pick — defaults keep today's prompts; under `auto` the availability probe and safety checks still run. See [docs/agents/claude.md](docs/agents/claude.md) "The default loop runner" |
