...
| `loopRunner`          | object  | rauf defaults           | Loop-runner binding for `forge-5-loop` (`bin`, command templates, `defaultAgent`, `minRunnerVersion` — floor **rauf ≥ 0.14.0**). Opt-in zero-prompt launch: `reviewMode` (`prompt`\|`always`\|`never`) gates the Run-mode question, `agentMode` (`prompt`\|`auto`) gates the agent pick — defaults keep today's prompts; under `auto` the availability probe and safety checks still run. See [docs/agents/claude.md](docs/agents/claude.md) "The default loop runner" |
...
| `docsStage`           | string  | `"prompt"`              | How `forge-6-docs`' Documentation Decision Gate behaves. `"prompt"` (default) asks generate-vs-skip at stage entry; `"skip"` records the stage as deliberately skipped (`state-skip`, `stage-exit --outcome skipped`) with no prompt, closing the pipeline honestly for teams that document differently. Docs that already exist are never silently reclassified. An unrecognized value behaves as `"prompt"`. |
