...
```bash
npx feature-forge install -a gemini
```

To see the exact destination on your machine without writing anything, run:

```bash
npx feature-forge install -a gemini --dry-run --json
```
...
1. List what got installed:
   ```bash
   npx feature-forge list -a gemini          # per-agent installed / up-to-date status
   ```
