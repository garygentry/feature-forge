...
This copies the generated `adapters/gemini/` bundle into Gemini's config directory:

```bash
node installer/dist/cli.js install -a gemini
```

To see the exact destination on your machine without writing anything, run:

```bash
node installer/dist/cli.js install -a gemini --dry-run --json
```
...
1. List what got installed:
   ```bash
   node installer/dist/cli.js list -a gemini          # per-agent installed / up-to-date status
   ```
