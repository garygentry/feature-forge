...
```bash
npx feature-forge install -a cursor
```

To see the exact destination on your machine without writing anything, run:

```bash
npx feature-forge install -a cursor --dry-run --json
```
...
1. List what got installed:
   ```bash
   npx feature-forge list -a cursor          # per-agent installed / up-to-date status
   ```
