...
| `config-local-ignored` | advisory | A present `forge.config.local.json` (the machine-local overlay, #324) is not git-ignored, so a per-machine override could be committed. | file absent, already ignored, or not a git work tree | add `forge.config.local.json` to `.gitignore` (`local-write`, no command — the remedy names the file) |
