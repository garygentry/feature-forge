...
- **`forge-guide --doctor`: the repair surface that turns a `doctor` finding into a consented fix (#244 P4, #254).** `doctor`'s `checks[]` (P0) and the Preflight & Self-Heal procedure (P1) gave the pipeline a health surface and a repair procedure, but only `forge-5-loop`'s gates 1c/1d and `forge-init`'s install preflight ever ran them — both narrowed to the handful of checks they gate on, and both a *stop*, not a repair. `/feature-forge:forge-guide --doctor` (D1: an argument on an existing skill, so the always-loaded `description:` budget stays at 4688 and no new skill is registered) runs the **full** catalog, renders it as id · status · severity · remedy tier with the blocking rows first, and then follows `references/preflight-and-self-heal.md` in full over the non-`ok` set — clustering findings that share a `remedy.command`, asking **one consolidated question per cluster** rather than one per check, running the command verbatim, and proving the repair by re-running the identical `doctor` invocation (a remedy that exits 0 but leaves a member check not `ok` is a failed preflight, never a partial success). The breadth is deliberate and is the one place `preflight-and-self-heal.md` §1's "never the full catalog" rule does not apply: that rule protects a *hard gate* from blocking a launch on an unrelated advisory, and this surface is *about* the environment rather than gating anything. forge-guide's standing "do NOT invoke stage skills or write files" rule is **narrowed, not lifted** — in this mode and nowhere else, `doctor` runs (read-only) and a **`local-write`** remedy applies only on an explicit yes; `global-install` and `network` stay **advise-only**, printed and never executed; stage skills are still never invoked, `.pipeline-state.json` is still never touched, and the skill still writes no artifact of its own. A non-interactive run is **report-only and says so**: the full-catalog report already carries P3.5's `interaction-mode` record, so the rung is read from `evidence.mode` before the first question rather than guessed at it — no second call, and no self-assessment of the half a model cannot see — and a rung-3 run states the ladder's own literal (`interaction: rung 3 (non-interactive) — declared defaults apply`), records every `local-write` cluster as `unaskable→advise-only`, and applies nothing that needed a yes. Reachability is the other half of the phase: the mode is now forge-guide's **first** Troubleshooting starter, `docs/doctor-checks.md` names it as the catalog's repair surface, and forge-5-loop's 1c/1d STOP text points at it (+5 words, inside the budget P1 freed) so a hard gate no longer ends by naming the failure without naming the fix. `tests/test_forge_guide_doctor.py` (new, 28 tests) guards the section, the narrowing carve-out clause by clause, the rung-3 literal *against the ladder at its source* (a non-vacuity check, so the pin cannot outlive the wording it pins), the untouched `description:`, the two pointers and forge-5-loop's pre-P1 slice budget, and — across all six bundles, resolving each host's own body filename rather than skipping the three that do not use `SKILL.md` — both the per-host command dialect (`/skill:forge-guide --doctor` on Pi) and the by-citation fan-out that puts `preflight-and-self-heal.md` beside forge-guide in every bundle. Prose and tests only: no script logic changed, `doctor`'s output is byte-identical, and INV-2 holds — a healthy project that never types `--doctor` sees nothing new.

- **The interaction rung and host are DATA a skill reads, not facts it guesses (#244 P3.5, #261, #263).** `doctor --json` gains a 16th check, **`interaction-mode`**, reporting `evidence.mode` (`interactive` / `non-interactive`, the latter carrying `rung: 3`) alongside `evidence.host` / `evidence.hostArg`, and `references/shared-conventions.md`'s Interaction Capability Ladder now tells every skill to read it instead of self-assessing. This closes the gap P3's smoke measured: five headless runs across `pi -p --mode json` and `codex exec` **each** self-assessed rung 2, emitted a prose question and stalled with every later step unreachable — on `main` as well as the P3 branch (pre-existing), and unmoved by a prose rewrite. It matters far beyond headless CI: `packages/loop/src/providers/presets.ts` in the runner gives every provider an explicit `nonInteractive` argv slot, so **every rauf loop iteration is a rung-3 session** — the exact mode P1 wired the preflight into at gates 1c/1d. The design rests on splitting what is observable: a model **can** see rung 1 vs rung 2 (does it have a structured question tool) and **cannot** see rung 2 vs rung 3 (is anyone there to answer), so the check supplies exactly the half the model cannot see and deliberately not the half it can — informing the ladder without becoming the host-implies-capability proxy INV-5 forbids. Detection is a three-tier precedence: the **`FORGE_INTERACTION`** launcher stamp (`interactive` / `non-interactive`; feature-forge only ever reads it, never sets it), then verified process ancestry, then `unknown`. **`unknown` is never reported as headless** — guessing headless would make an interactive session silently skip its questions and take no-write defaults, trading a visible stall for a silent behavior change, so `unknown` is an `na` record meaning "self-assess as before". Ancestry is deliberately asymmetric (a verified harness ancestor carrying `-p`/`--print` is non-interactive; one exposing arguments without a headless flag is interactive) and refuses a mode in two measured cases. A harness that exposes **no arguments at all** yields no mode: Pi overwrites its own argv with its process title, so `/proc/<pid>/cmdline` for a `pi -p --mode json` session reads back as exactly `pi` with the flags erased — the first P3.5 smoke run stalled at rung 2 because the detector read that as *interactive*, a confident wrong answer in the dangerous direction (this also stops a wrapper's argv being borrowed: `timeout 600 pi -p …` puts `-p` in `timeout`'s command line, and under the loop the parent is the runner's `node` process, which carries no such flag at all). And ancestry **excludes Codex entirely**: a spike disproved every subprocess-observable Codex signal — `isatty()` is false even in a fully interactive session, `CODEX_CI` and `TERM=dumb` are injected into *every* Codex tool call in both modes (A/B verified), and Codex's shell tool executes under a long-lived, init-parented `app-server` daemon predating the session, so ancestry there describes the daemon's launch mode rather than the session's. A `codex` ancestor therefore still identifies the *host* but never the *mode*; the env stamp is the only signal that reaches Codex, and it was verified live through `codex exec --ignore-user-config` under `workspace-write`. Ancestry therefore answers for **Claude** in practice (verified live: `claude -p` → `non-interactive`, `rung 3`, `host claude`, via ancestry) and identifies the host but not the mode for Pi and Codex, both of which the launcher stamp covers on every loop iteration. `evidence.ancestry` carries only each ancestor's pid, executable basename and headless flags — never raw argv, with a basename that does not look like an executable redacted to `?`, since a parent's command line can carry a username or a credential and evidence lands in output that gets pasted into issues. The check spawns **nothing** (one env read plus `/proc` reads). Two signals that *positively contradict* each other resolve to **neither**: the record is `warn` with `evidence.conflict` and a mode of `unknown`, so skills self-assess (ask) rather than skip a question — `warn` also being what gets it printed on the human path, where `ok` records are suppressed. And `--print` alone no longer proves a missing reply channel: Claude's Agent SDK drives `--print` with `--input-format stream-json` / `--permission-prompt-tool`, a bidirectional session whose host *can* answer, so those flags withdraw the headless claim. The bundle sentinel beside the script (`adapters/<agent>/.feature-forge-bundle.json`) supplies the host exactly, closing the second half of #261: canon repeatedly says "gate on the build-substituted `--host`, never on tool availability", yet no file a skill loads carried a concrete value — so the only signal in front of an agent was the one INV-5 forbids, and on Pi it says "Claude". Also **fixes #263**: `references/preflight-and-self-heal.md`'s scope line named only `forge-5-loop`'s 1c/1d, which P3 made stale by adding `forge-init` as a second caller, and the ladder now states that **"no-proceed" scopes to the decision in front of you, never to the skill** — a rung-3 site declines *that action* and continues, since a skill that quietly ends early is indistinguishable from one that finished. The Codex and neutral host overlays, which previously asked an agent to judge its own mode (inert by construction — run 5 had Codex report `rung 2 (interactive Codex)` *while running under `codex exec`*), now defer to the record, and Pi's overlay notes that its stripped tool's absence cannot decide the rung while keeping the `Error: UI not available (running in non-interactive mode)` backstop for a tool that is present but errors. `tests/test_doctor_checks.py` adds 16 tests over a synthetic `/proc` (nested sessions, cycles, an absent `/proc`, the redaction, the Codex refusal) and `tests/test_adapter_host_neutrality.py` adds INV-10 assertions that all six bundles carry the pointer, the launcher contract and the never-headless rule. No skill body changed (INV-7 net zero), frontmatter stays 4688, and no check emits `fail`.

- **Tooling Feedback Protocol: `forge-init` writes the project-root feedback section and preflights `gh` (#244 P3, #253).** The `specs/` hygiene files forge-1-prd and forge-0-epic drop into a project already deferred to a project-root `AGENTS.md`/`CLAUDE.md` "Tooling feedback" section for the full flow — but only `forge-bootstrap` ever wrote that section, so in every `forge-init` project the pointer named a file that did not exist (G9: the defect was a **dangling pointer**, not a missing protocol). `references/shared-conventions.md` gains `## Root Hygiene (Tooling Feedback)`, the one canonical statement of the protocol — route by tool (feature-forge vs. rauf issue tracker), capture *what you ran / what you expected / what actually happened / a fix idea* while fresh, file with `gh issue create` **only on the human's explicit go-ahead**, and **never mid-loop** (an autonomous rauf iteration appends to `progress.md` instead) — plus the idempotent `[ -f … ] || cp` copy contract it shares with Specs Directory Hygiene. `references/templates/root-hygiene/{AGENTS,CLAUDE}.md` (new) ship that section as project content, **lifted verbatim** from `skills/forge-bootstrap`'s hygiene templates rather than rewritten, with a test asserting the two stay byte-equal so the copies cannot drift. `skills/forge-init/SKILL.md` gains two steps: a narrowed preflight (`doctor --json --check plugin-root --check root-version-skew --check gh-available`, followed per `references/preflight-and-self-heal.md` — `gh`'s remedies are `global-install`/`network`, so they are advise-only and nothing is executed, which is what makes the protocol degrade to "here is the issue text, file it yourself" instead of proposing a command that cannot run), and a root-hygiene step that points at the new block by title. The write is a **`local-write`**: one consolidated question covering both variants, then the guarded copy — never overwriting an existing root file (if one exists, the block is printed to paste), with `AGENTS.md` always offered and `CLAUDE.md` only on the Claude host, chosen from the build-substituted `--host` and never from question-tool availability (INV-5). Its rung-3 default is the no-write option, stated (INV-6). `scripts/forge-bootstrap.py` records that `_write_artifact`'s existing never-overwrite guard is what keeps the two writers from duplicating each other's block, and the specs-hygiene templates' pointer is reworded to name who writes the root file ("written by forge-init or forge-bootstrap; if absent, run `/feature-forge:forge-init`"). `tests/test_forge_init_tooling_feedback.py` (new) guards the protocol's clauses, the verbatim lift, the guarded copies, the host gating, the rung-3 default and the closed pointer; `tests/test_adapter_host_neutrality.py` adds INV-10 assertions that all six bundles carry the section and ship the new templates with no host-term degradation — byte-identical to canon on five, and on Pi identical once its bundle-wide `/feature-forge:` → `/skill:` substitution is undone, which is correct for a template landing in a Pi-driven project (`templates/` is translation-exempt — a host-term pass would rewrite the `/feature-forge:*` the CLAUDE.md variant deliberately names). Prose + templates only; no pipeline logic changes and forge-init still writes `forge.config.json` exactly as before.

- **Interaction Capability Ladder: rungs 1/2/3 and conservative rung-3 defaults, generalizing forge-init's prior art across canon (#244 P2, #252).** `references/shared-conventions.md` gains `## Interaction Capability Ladder`, the single canonical statement of interaction capability (roadmap §5.4): rung 1 (a structured question tool is present — use it), rung 2 (no structured tool, but the host can still prompt and wait — Codex interactive), and rung 3 (genuinely non-interactive — print/JSON mode, headless CI). Two independent axes: `--host` is static per adapter bundle (the build-substituted value) and never implies a rung either direction (INV-5); rung is dynamic, self-assessed once per turn exactly as `--verify-capability` already is, and stated once in output whenever it is not rung 1. Per-host rung-3 detection: Claude (question tool absent from the tool surface, or no TTY), Codex (`codex exec`), Pi (`AskUserQuestion` stripped from the tool list in `-p`/`--mode json`; a call attempted anyway fails with `Error: UI not available (running in non-interactive mode)`, never read as a user decline). The rung-3 rule is conservative-only (INV-6): take the declared default — always the no-write / no-proceed option, never advancing a pipeline stage, launching a loop, applying a remedy, creating a branch, committing, or recording a decision — and state it, or `no-default: abort — <question> requires a human answer` for an undefaultable interview question; either outcome must be stated in the output. Because forge-verify has zero free body lines, a **site-class table** replaces 107+ per-site declarations (verification warnings → STOP + print the verify command; dependency/epic gates → STOP; launch/run-mode → STOP + print the rendered command; preflight remedies → `local-write` degrades to advise-only; branch setup/reconcile → never create/switch/record; interviews/pickers/blocking gates → `no-default: abort`; entry/completion guards → STOP; destructive confirmations → never mutate; verify offers → fall back to `--verify-capability manual`; recovery interview → fall through to `needs-human`) — every prompting surface points at the ladder by title rather than restating its row. Softens the User Input Protocol's absolute `AskUserQuestion` mandate to point at the ladder for rung 2/3 without weakening the rung-1 MUST on a host that has the tool, and fixes both host-proxy conflations INV-5 forbids (shared-conventions.md's Epic Context Injection block and forge-bootstrap's `Answers.host` field both previously used question-tool *availability* as a stand-in for host identity — a real bug for the latter, since Pi's `AskUserQuestion`-compatibility extension would misidentify a Pi bootstrap run as Claude). Annotates rung-3 defaults at the five highest-traffic uncapped sites (forge-5-loop's recovery-procedure.md, runner-contract.md, agent-selection.md; forge-0-epic's epic-manifest-subcommands.md; stage-exit-protocol.md's capability-determination clause), rewrites the Codex/neutral build-adapters.py overlays to be rung-aware (replacing the unconditional "never skip a required question" with "ask & wait at rung 2, take the declared default and state it under `codex exec`"), adds a Pi non-interactive overlay bullet, gives forge-bootstrap a rung-3 interview sentence (its multi-question interview has no sane unattended default), and swaps forge-init's own local three-case description for a citation — it was the prior art the ladder generalizes, so it becomes the reference implementation rather than a duplicate. `tests/test_interaction_ladder_prose.py` (new) guards the canonical section's clauses and a **computed** roster (every `skills/**/*.md` + `references/**/*.md` containing `AskUserQuestion`, floor 15) rather than a hardcoded site list, with a strict title-pointer rule (a bare `shared-conventions.md` citation does not count). `tests/test_preflight_self_heal.py` gains a rung-3 cross-reference check now that P1's interim rule points at the now-canonical ladder instead of carrying a stale "(interim)" caveat. Prose-only, zero canon logic change: INV-2 holds, a healthy Claude session sees no behavioral difference.

- **Preflight & Self-Heal procedure, wired into forge-5-loop's runner gates (#244 P1, #251).** New `references/preflight-and-self-heal.md` names the seven-step shape (enumerate → cluster → consolidated prompts → record → apply → prove → return) that turns a `doctor` `checks[]` finding into a clear stop or a scripted, consented repair, mirroring `recovery-procedure.md`'s proven failure rule (a remedy that errors, times out, or does not prove itself on re-check is surfaced verbatim and reported as a **failed preflight**, never as healed) narrowed to remedies rather than backlog decisions — it deliberately never touches the item-keyed decision record post-run recovery uses, since an environment fix is not a backlog decision. `references/shared-conventions.md` gains `## Remedy Safety Ladder`, the single canonical statement of the four remedy tiers (`read-only` run freely, `local-write` asks once per distinct `remedy.command` per session then runs, `global-install`/`network` advise-only and never executed) plus the "degrade one tier stricter when the session cannot ask" rule (disambiguated for a Codex-shaped host: an interactive session that merely lacks a structured question tool can still ask in prose and wait — the stricter default fires only for a genuinely non-interactive invocation), referenced by `safety` value everywhere a remedy applies. forge-5-loop's gates 1c/1d (`skills/forge-5-loop/SKILL.md`) are rewritten to run `doctor --json --check runner-binary --check runner-version --check runner-wired --check runner-legacy-layout` (explicit ids, never a glob) and follow the new reference — `runner-binary`/`runner-version` not `ok` is still a HARD GATE FAILURE, now showing the failing check's own `remedy.description` rather than a hardcoded `loopRunner.installHint` (three of `runner-binary`'s four warn shapes are a config fix, not an install — G4); `runner-legacy-layout` still STOPs with the Ralph-migration text, now naming the check's own `remedy.command` instead of a hardcoded `rauf migrate .` (a customized `loopRunner.bin` changes the binary name); `runner-wired` still STOPs with `setupHint`. Condenses the 1c/1d gate span from 26 lines / 329 words to 16 lines / 188 words (body net −10 lines / −141 words, from 296/4990 to 286/4849 — all of it available to later phases; INV-7). Zero behavior change on a healthy project: all-`ok`/`na` proceeds silently, exactly as today.

- **`doctor` is a real health surface: a registry of 15 structured checks, warn-only, with remedies as data (#244 P0, #250).** `python3 scripts/forge-session.py doctor --json` now appends `checks[]` — one `{id, status, severity, detail, evidence, remedy}` record per registry entry, `status ∈ ok|warn|fail|na` — plus `checksSummary` and `remedyClusters[]` (identical remedy commands merged at their most conservative safety tier). The eleven legacy fields are unchanged and still first, so existing consumers keep working. The registry (`docs/doctor-checks.md`) promotes the five ad-hoc fields (`plugin-root`, `config-schema`, `backlog-present`, `branch-state`, `sandbox-root`) and adds ten: `root-version-skew`, six `runner-*` checks over the loop runner and its `.rauf.json` (binary on PATH — a customised `bin` missing while the default is present is reported as a config fix, never the install hint; `version --json` vs `minRunnerVersion`; wired; legacy Ralph layout; `installedBy` vs live version; `testCommand` vs the profile's `commands.test` **or** sibling `verify`), `config-completeness` (per-feature required keys keyed on the next stage), `backlog-valid` (`{bin} backlog validate`, once per distinct backlog dir, for loop-ready features) and `gh-available` (`gh --version` + `gh auth token` exit code, stdout to `/dev/null`; never `gh auth status`, which tests the network). The contract: doctor still exits 0 for everything it finds (a crashing check is an `na` record, a crashing driver degrades every check to `na`; the legacy loaders now also treat non-UTF-8 config/state files and a non-string `pipelineStatus` as data instead of tracebacking); **no check emits `fail`** in this release (`FAIL_PROMOTED_CHECK_IDS` is empty and the driver demotes any `fail` to `warn`); every `remedy` is `{description, command, safety}` data the operator runs — doctor never executes one, never runs `rauf update --check` or the runner's `agentsProbeCommand`, and never touches the network (proven three ways in `tests/test_doctor_checks.py`: an argv allowlist over scrubbed-PATH fakes, an in-process `subprocess.run` recorder asserting every spawn is on the allowlist and equals no emitted remedy command, and a run under `unshare -rn` with identical verdicts). Every probe has a 10 s timeout that degrades to a `warn` with `evidence.timedOut`. CLI: `--check ID` (repeatable; a typo is an argparse error) narrows the registry, `--verbose` prints the `ok`/`na` lines, `--schema` overrides the bundled config schema for tests; the human report ends with a `checks: N ok, N warn, 0 fail, N na` line and `! id: detail` / `remedy [tier]: …` pairs. Zero canon churn: no skill or reference body changed, and the forge-verify smoke (`doctor --json`) is unaffected.

- **Machine-local runner override: `forge.config.local.json` + `FEATURE_FORGE_LOOP_RUNNER_BIN` (#324).** A gitignored per-machine config layer overrides `loopRunner.bin` (and the env var overrides both) without a tracked diff, for pointing a dev box at `rauf-dev`/`rauf-stable`; `doctor --json` reports the effective config's `layer` provenance and warns (`config-local-ignored`) when a local file is present but shadowed.
- **Explicit `FEATURE_FORGE_ROOT` beats plugin-root discovery (#323).** Setting `FEATURE_FORGE_ROOT` pins the resolved plugin root ahead of auto-discovery (byte-identical prelude across every skill fence + `forge-root.sh` Step 0); an invalid value fails the prelude with a message, and `doctor` reports the resolution `channel` (with `--explain` / `--explain --all`).

### Changed

- **rauf pin advanced to `@garygentry/rauf@0.16.0`.** rauf 0.16.0 ships `rauf version --json`
  provenance (`channel`/`path`), a `FORGE_INTERACTION=non-interactive` loop-child stamp, Codex
  marketplace install, and install-binary/loop-runner polish — surfaces feature-forge's doctor
  *may* read but no stage *requires*, so `minRunnerVersion` stays at `0.14.0`; only the
  provisioned default pin advances.
- **Source-dogfood docs for Codex and Pi; `dev-plugin.sh` scoped to Claude (#315).** Documents how to run the pipeline from an un-built canon checkout on each host (`docs/DOGFOODING.md`) — the built bundles already fan shared references skill-local per host (#132), so only the canon-load path needs care — and narrows `scripts/dev-plugin.sh` to the Claude host it actually supports.

### Fixed

- **The built Claude bundle ships its own `.claude-plugin/plugin.json` (#322).** `scripts/build-adapters.py` now emits the plugin manifest into `adapters/claude/`, so the built bundle is a valid plugin on the marketplace and `--plugin-dir` load paths (previously only canon carried it). Nested-manifest safe: the CC marketplace is manifest-driven/root-only and `--plugin-dir` reads one level deep.
- **CI guards shared-reference resolution on the marketplace canon channel (#314).** An xfail-strict test pins the known gap that a marketplace `source:"."` (un-built canon) channel does not resolve a skill's shared `references/X` fan-out (46 unresolved), and flips to pass at `./adapters/claude`; the root fix is deferred, but a regression that silently resolves it wrong now fails CI.
- **`/skill:forge-*` invocation arguments are preserved deterministically on the Pi host (#303).** Pi expands `/skill:<name> <args>` by appending the raw argument string after the skill body with only a bare `\n\n` separator — no semantic boundary. Pi's docs say arguments arrive as `User: <args>`, but the implementation has bare-appended since Pi v0.50.0 (commit `7868b25a2`), still true through the installed `0.84.4` and tested `0.85.1`. So `/skill:forge-1-prd rendered-model-v2` reached the model with the feature name as unlabeled trailing text, and the model could read it as prose and wrongly trip feature-forge's "no feature name → STOP and ask" rule — losing a token that keys directory resolution, branch naming, and every `.pipeline-state.json` write. A new first-party Pi compatibility extension, **`forge-invocation-args`**, hooks Pi's `input` event (which fires *before* skill expansion, and after extension-command dispatch, so `/skill:` reaches it) and rewrites a matched `/skill:forge*` invocation to inject a versioned envelope — `<feature-forge-invocation version="1"><skill>…</skill><arguments>…</arguments></feature-forge-invocation>` — that Pi's built-in expansion then appends as the skill's arguments. The `<arguments>` payload is XML-entity-escaped (`&`/`<`/`>`), which is lossless and collision-proof (a literal `</arguments>` cannot close the element), so zero/one/multiple arguments, flags, and multiline text all survive; the envelope is always emitted (empty `<arguments>` ⇒ no feature name supplied). Inject-into-args was chosen over replicating Pi's private skill-block wrapper because it depends only on Pi's *documented* contract (args are the remainder after the command), verified against Pi's `_expandSkillCommand`. Extension-injected messages (`source: "extension"`, dispatched with `expandPromptTemplates: false`) are left untouched, the handler is idempotent (never double-wraps), and it **fails loudly** via `ctx.ui.notify` rather than throwing — a thrown input handler would make Pi fall back to the original untransformed text and silently reintroduce the bug. A host-neutral rule in `references/shared-conventions.md` § Feature Name Requirement teaches every forge skill to read the feature name from the envelope (defense in depth; harmless no-op on non-Pi adapters). Restoring an explicit boundary upstream in Pi is tracked separately; this workaround remains useful host-compatibility hardening regardless.
