...
- **Pi's `AskUserQuestion` is now a vendored upstream package rather than a bespoke implementation.** The hand-written 446-line extension is replaced by a vendored snapshot of [`@juicesharp/rpiv-ask-user-question`](https://www.npmjs.com/package/@juicesharp/rpiv-ask-user-question) 2.1.0 (MIT), carried under `adapter-src/pi/extensions/ask-user-question/` with a documented four-patch delta — see `adapter-src/pi/UPSTREAM.md`. Users gain per-option notes, a collapsible overlay, localized chrome, terminal-row-aware overflow scrolling, and an **RPC/ACP fallback**: on hosts that report a UI but cannot render a custom overlay (the VSCode pendant, Zed, Paseo), the questionnaire now degrades to sequential `select`/`input` dialogs instead of returning "the user never saw the questions", which previously stalled the pipeline's interview stages on those hosts. The tool keeps Claude's `AskUserQuestion` name — vendoring puts the name under our control, so canon needs no build-time rename and all existing skill prose stays correct. It is vendored rather than depended on because `adapters/pi/` must stay self-contained: the interview stages have no fallback question mechanism on Pi, so a missing `pi install` would be a hard stall rather than a degraded experience.
...
- **`adapter-src/pi`'s `verify` gate now runs behaviour, not just types.** Alongside `tsc --noEmit` over the whole vendored tree, `node --test` drives the real extension through a fake `ExtensionAPI` and a headless TUI: registration and tool name, the questionnaire state machine (tabs, single- and multi-select, preview pane, cancel), the RPC fallback, and the validation guards. These assert *feature-forge's* contracts rather than restating upstream's own suite, so the gate that matters is "did a refresh change what we depend on". It earned its place on first run by catching a bundle-root resolution bug introduced by the source layout.
- **Hand-written adapter sources are now verified before they ship.** The Pi `AskUserQuestion` extension previously lived as a 448-line TypeScript string literal inside `scripts/build-adapters.py`, where it was in no `tsconfig` and so could not be typechecked, linted, or built — no typo, bad import, or type error in the most logic-dense artifact the generator emits could surface before a Pi user hit it at runtime. Real code that a target agent loads now lives at `adapter-src/<agent>/<file>`, is read by the generator at build time, and is verified by a new `scripts/validate.sh` step that iterates `adapter-src/*/` and runs each directory's own `verify` script (Pi's is `tsc --noEmit` against Pi's published type declarations). An agent directory with no verifier is reported as a visible `SKIP`, never a silent pass. Typechecking the extracted source immediately surfaced three latent type errors, all rooted in an untyped `ctx` parameter, now fixed by using Pi's exported `ExtensionContext` type. Generated output is unchanged apart from those annotations and the provenance header, which now names its `adapter-src` path.
...
- The generated Pi `AskUserQuestion` fallback now uses the promoted ghoseb-style interaction model: explicit option selection, tabbed multi-question navigation, final Submit review, focused previews, stable multi-select ordering, explicit Other/custom option dedupe, delayed registration, and skip-on-existing-tool coexistence with standalone Pi packages.
