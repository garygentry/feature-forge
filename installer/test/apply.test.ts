...
function ctxFor(
  ...
  return {
    ...
    source: src,
    raufPin: "@garygentry/rauf@0.14.0",
    now: NOW,
    priorManifest: null,
    ...extra,
  };
...
test("copy: an all-unchanged re-run with a CHANGED raufPin rewrites the manifest (F1)", async () => {
  await withSandbox(async (sb) => {
    ...
    const first = planInstall({
      agent: "claude", scope: "project", mode: "copy", destination: ctx.destination,
      source: src, priorManifest: null, force: false, raufPin: "@garygentry/rauf@0.14.0",
    });
    assert.ok(first.ok);
    await apply(first.value, ctxFor(sb, "claude", src, "copy", { raufPin: "@garygentry/rauf@0.14.0" }));

    const m1 = readManifest(ctx.manifestPath);
    assert.ok(m1.ok && m1.value !== null);
    assert.equal(m1.value.raufPin, "@garygentry/rauf@0.14.0");

    // Re-plan: files all unchanged, but the pin was bumped.
    const second = planInstall({
      agent: "claude", scope: "project", mode: "copy", destination: ctx.destination,
      source: src, priorManifest: m1.value, force: false, raufPin: "@garygentry/rauf@0.14.0",
    });
    assert.ok(second.ok);
    ...
    const report = await apply(
      second.value,
      ctxFor(sb, "claude", src, "copy", { priorManifest: m1.value, raufPin: "@garygentry/rauf@0.14.0" }),
    );
    assert.equal(report.ok, true);

    const m2 = readManifest(ctx.manifestPath);
    assert.ok(m2.ok && m2.value !== null);
    assert.equal(m2.value.raufPin, "@garygentry/rauf@0.14.0", "pin change persisted despite all-unchanged files");
  });
});
