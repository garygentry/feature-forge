...
try:
    ...
    # routing read, so a bare-copy stage-exit run — the `_stub_bundle` tests — needs
    # them bound). The doctor verb itself needs the package (a bare copy cannot run
    # it, and the doctor tests copy the package beside this shim). The package-present
    # branch above is authoritative; these are byte-equal fallbacks, never a second
    # source.
