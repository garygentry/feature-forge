...
GEMINI_EXTENSION_VERSION: str = "0.20.0"
...
# Runtime helper PACKAGES copied WHOLE (every module) into every bundle alongside the
# single-file RUNTIME_HELPERS. `forge-session.py` is a thin shim that imports the
# `forge_session/` package sitting next to it (#279), so the package must ship with it
# or the bundled CLI cannot import its own internals. Each package module is asserted
# byte-identical to canon, exactly like the single-file helpers (REQ-GEN-04/05).
# `__pycache__` is never copied — generated bytecode is not a shipped surface.
...
_HOST_TERM_REPLACEMENTS: tuple[tuple[str, str], ...] = (
    ...
    # Bootstrap-prelude root hint: as of #323 the prelude leads with the neutral
    # `${FEATURE_FORGE_ROOT:-}` candidate, so no body translation of the Claude hint is
    # needed. Every bundle keeps `${CLAUDE_PLUGIN_ROOT:-}` as the SECOND candidate — on a
    # non-Claude host it simply expands to empty and is skipped (harmless), and no
    # host-neutrality token forbids it. (Rewriting it to a second `${FEATURE_FORGE_ROOT:-}`
    # would only duplicate the first candidate.)
...
_HOST_NOTES_CODEX = (
    ...
    "- **User input:** Codex has no structured question tool. Which rung you are on is "
    "**not** observable from inside the session — a `codex exec` run looks exactly like "
    "an interactive one — so read it from the Interaction Capability Ladder's "
    "`interaction-mode` record instead of judging it. Interactive session — "
    "ask the question directly and wait for the reply; never assume an answer. "
    "Non-interactive — don't wait: take the Interaction Capability "
    "Ladder's declared conservative default, state it in your output, and use "
    "`no-default: abort — <question> requires a human answer` for an interview "
    "question with no sane default (`references/shared-conventions.md`).\n"
...
_HOST_NOTES_NEUTRAL = (
    ...
    "runtime can prompt and wait; never assume one. Read whether it can from the "
    "Interaction Capability Ladder's `interaction-mode` record rather than judging it. "
    "When your runtime is genuinely "
...
_HOST_NOTES_PI = (
    ...
    "- **Non-interactive (`-p`/`--mode json`):** `AskUserQuestion` is stripped from "
    "the tool list, so its absence alone cannot tell you the rung — read that from "
    "the Interaction Capability Ladder's `interaction-mode` record. A call attempted "
    "anyway fails with `Error: UI not available "
    "(running in non-interactive mode)` — never read that as a decline. Take the "
    "Interaction Capability Ladder's declared conservative default, state it in your "
...
def run_self_containment_pass(
    ...
    # (3b) Runtime helper PACKAGES — the whole `forge_session/` tree beside forge-session.py
    #      (#279). Same byte-identical contract as the single-file helpers above; the shim
    #      imports this package at runtime, so a bundle missing it cannot run its own CLI.
    for pkg in RUNTIME_HELPER_DIRS:
        src_pkg = repo_root / "scripts" / pkg
        for src_file in sorted(src_pkg.rglob("*.py")):
            if "__pycache__" in src_file.parts:
                continue  # generated bytecode is not a shipped surface
            rel = src_file.relative_to(repo_root / "scripts")
            dst_file = bundle_root / "scripts" / rel
            dst_file.parent.mkdir(parents=True, exist_ok=True)
            _assert_within(dst_file, bundle_root)
            shutil.copyfile(src_file, dst_file)  # bytes only — never copystat/edit
            dst_file.chmod(0o644)
            _assert_byte_identical(src_file, dst_file)  # REQ-GEN-05 hard assertion
