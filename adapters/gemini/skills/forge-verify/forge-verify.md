...
**Outcome.** Derive the `--outcome` from the settled on-disk record, never by eyeballing conversational state. After Step 6's `state-verify` write, run `select-outcome` — it reads the verify entry and the latest report and returns `{outcome, reason, evidence}`, the `outcome` being exactly one of `passed`, `findings`, `skipped`, or `failed`. Add `--op-failure` when a dispatch, check, or state write failed this run (a `CLEAN_ROOM_UNAVAILABLE` sentinel or a non-answer counts): it is the one signal disk cannot record. On exit 2 the disk names no terminal result and no signal was given — surface its `Error:` line rather than guessing. The four-way mapping and its rationale live in `references/select-outcome.md`.
