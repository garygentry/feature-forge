...
### CRITICAL GUARDRAIL: Use host's question mechanism for All Questions
...
**Required turn structure:** Output your analysis, findings, or context as regular text. Then call host's question mechanism with your questions. Do NOT mix questions into your text output.
...
[then call host's question mechanism with: "1. Where should this component live? 2. Should we use server-side rendering?"]
```

### Decision Support: Help the User Choose

When a question posed through host's question mechanism carries substantive options (a real choice — not a trivial yes/no confirmation), do not just list them. The interview stages have already done codebase research and integration analysis; surfacing that synthesis at the decision moment is the whole point. For every such question:
...
**Site classes.** Rather than a per-site declaration (107 sites and rising), every question surface that uses host's question mechanism inherits its rung-3 default from its class — a surface cites this table by title, never restating its own row:
...
- **Blocking review (gate).** The stage presents the artifact and collects feedback through host's question mechanism; it does **not** proceed until the user answers, iterating until they confirm. Stages: **forge-1-prd** (Step 5), **forge-2-tech** (Step 6), **forge-3-specs** (Step 6).
...
- `autoInvokeNextStage` (default: `true` — the `forge` navigator auto-invokes the next stage via the host's skill-invocation mechanism after the user confirms; `false` keeps copy-paste behavior. Navigator-only.)
...
4. **If the current branch IS the default branch** → use host's question mechanism with a **strong recommendation** (still optional):
...
- **`warn-drift`** — you are on the **default** branch and the state records a topic branch. Via host's question mechanism, strongly recommend creating/switching to `{branchPrefix}{feature}` (then record it), still allowing **proceed on the default branch**. Never hard-stop.
