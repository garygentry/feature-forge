...
|-----------|---------------|----------------|
| Epic name duplicates an existing name | `check-name` exit 1 (`duplicate-name`) | STOP creation; surface finding; ask for a new name via host's question mechanism |
...
| Bad `--status` value | `set-status` exit 2 (argparse) | Surface; re-prompt via host's question mechanism with the valid choices |
...
names host's question mechanism — never invent a name or a `--status` value to keep the mutator
moving unattended.
