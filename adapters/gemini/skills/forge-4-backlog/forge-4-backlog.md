...
1. **Verify the backlog first — if it isn't already verified.** When this stage has no
   ...
   **now, before clearing**. If verify already ran, is pending under auto-verify, or
   ...
   subagent):
   - **Verify the backlog now** *(recommended)* — dispatch the clean-room `forge-verifier`
...
3. **Then run `/feature-forge:forge-5-loop {feature}`** in the fresh session — or re-run `/feature-forge:forge`
