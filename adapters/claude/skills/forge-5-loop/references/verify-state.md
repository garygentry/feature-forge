...
```json
{
  ...
  "verified": false,
  "stale": true,
  "message": "Fixes were applied to auth's forge-4-backlog but nothing re-verified them; re-verification is still outstanding — run /feature-forge:forge-verify auth backlog.",
  "nextCommand": "/feature-forge:forge-verify auth backlog"
}
```
