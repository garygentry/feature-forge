...
from typing import Callable, Final, Literal, NamedTuple, NoReturn, get_args
...
try:
    ...
    # stays runnable, so define just the two primitives the non-topology paths touch
    # at import/run time (UsageError is raised/caught by the write + exit verbs;
    # VerifyStatus is read by get_args at module load). The package-present branch
    # above is authoritative; these are byte-equal fallbacks, never a second source.
...
def _check_backlog_valid(ctx: _CheckContext) -> dict:
    ...
    return _result(
        ...
        + " — fix the findings or re-run /feature-forge:forge-4-backlog <feature>",
        evidence,
    )
...
def _check_backlog_present(ctx: _CheckContext) -> dict:
    ...
    return _result(
        ...
        "/feature-forge:forge-4-backlog <feature> to regenerate it",
        evidence,
    )
...
def verify_state_for_stage(
    ...
    verify_key = f"forge-verify-{token}"
    command = f"/feature-forge:forge-verify {feature} {token}"
    feature_dir = _resolve_feature_dir(specs_dir, feature, epic)
    state = _read_state(feature_dir / PIPELINE_STATE_FILENAME)
    entry = _verify_entry(state, verify_key)
    raw_status = entry.get("status")
