...
question goes through host's question mechanism. **Rung-3 default**
...
`blocksCurrent`, `raisedBy`), then for **each** request use host's question mechanism to offer **Apply**,
...
- **Apply — composite kinds (`move-boundary`, `split`):** there is no single mutator (v1). Walk the
  ...
  host's question mechanism; never batch-apply.
...
Use host's question mechanism to offer the edit operations, each mapping to one helper mutator:
...
`in-progress` or `complete`, use host's question mechanism with an explicit warning naming the affected
...
user request**: offer it via host's question mechanism but default to the targeted patch. The skill keeps
...
- **`actionable` is empty** → there is no member to hand off to. **Omit `--next-feature` entirely**; the script decides the terminus from live epic state — a **finished** epic closes terminally (`primaryCommand: null`, nothing fenced), anything else routes to the epic dashboard. Print whichever block it emits verbatim; do not add a command to a terminal one. Never substitute a stand-in for the flag value — not the literal `{first-actionable-feature}`, not the epic name, not an arbitrary member off `features[]`. An unresolved placeholder is not a member name, and the script rejects an unsafe value with exit `2`.
