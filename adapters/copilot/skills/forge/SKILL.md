...
name: forge
description: 'Feature-forge pipeline navigator and status dashboard. Use when the user references the forge pipeline, asks about forge status or progress, types invoke-skill: forge, or wants to check what stage a feature is at in the forge pipeline. Do NOT use for general feature requests, project status, or tasks unrelated to the forge development pipeline.'
...
**If a feature name is provided** (e.g., `invoke-skill: forge auth`):
...
- Otherwise, resolve the name via the **Feature Directory Resolution** block in `references/shared-conventions.md` (so a nested epic-member name finds its dashboard too). On a resolution failure (`not-found` / `ambiguous` at exit 1; `unsafe-name` or a path-containment escape at exit 2), surface it verbatim.
  - On `not-found`, first run the cross-branch discovery step from that same Feature Directory Resolution block (`forge-session.py discover-feature` — the state may live on a topic branch or an unfetched remote branch). Candidates → offer switch / fetch+switch per that block (explicit accept + clean tree only), then re-resolve and render the dashboard. Only when discovery also returns nothing, ask: "No pipeline exists for '{feature}' on any branch. Want to start one? Run `invoke-skill: forge-1-prd {feature}` to begin." Never render a dashboard from memory of earlier sessions (anti-fabrication guard).
...
Next: Continue with invoke-skill: forge-3-specs {feature}
      Or verify tech-spec with invoke-skill: forge-verify {feature}
...
**4. Act on the choice.**
- **Clean session** (the default) → give the exact next command and the session clear-then-re-run instruction; do not invoke anything (you cannot clear your session / start a fresh session for the user).
...
This applies whether the feature was named explicitly (`invoke-skill: forge {feature}`) or resolved from the recency default.
...
Next: invoke-skill: forge-3-specs token-service
⚠️ Pending epic changes: token-service (1). Run invoke-skill: forge-0-epic auth-overhaul to reconcile.
```
...
```
Commands:
  invoke-skill: forge-1-prd <feature>      Create requirements document
  invoke-skill: forge-2-tech <feature>     Create technical spec
  invoke-skill: forge-3-specs <feature>    Create implementation specs
  invoke-skill: forge-4-backlog <feature>  Generate rauf backlog
  invoke-skill: forge-5-loop <feature>  Run rauf autonomous loop
  invoke-skill: forge-6-docs <feature>     Generate architecture docs
  invoke-skill: forge-verify <feature>     Run verification on current stage
```
...
- `invoke-skill: forge pause {feature}` — Set `pipelineStatus` to `"paused"`. Do NOT modify `currentStage` or any stage statuses. The pipeline freezes exactly as-is. Show a confirmation.
- `invoke-skill: forge resume {feature}` — Set `pipelineStatus` back to `"active"`. Calculate how long the feature was paused (from `updatedAt` to now). If paused for more than 24 hours, show a hint: "This feature was paused for {duration}. Session context may have been lost — consider re-running `invoke-skill: forge-{currentStage} {feature}` to rebuild context."
- `invoke-skill: forge abandon {feature}` — Set `pipelineStatus` to `"abandoned"`. Use the host's question mechanism to confirm first, and state what's reversible: abandoning does not delete artifacts and can be undone with `invoke-skill: forge resume {feature}`, so the cost is low — but if the user really means "stop and discard," point out that `pause` is the better choice when they're only setting it aside. Offer **Abandon** · **Pause instead** · **Cancel**.
- `invoke-skill: forge run [{feature}]` — **Opt-in auto-advance.** Drive the feature through consecutive stages in one session instead of confirming each boundary. This is a convenience wrapper over **3b. Drive to the Next Stage** — same stage order, same context gate — just looped:
...
When listing features, show active pipelines by default. Include a count of paused/abandoned: "3 active pipelines (1 paused, 1 abandoned — use `invoke-skill: forge list all` to see them)."
