...
**In-stage auto-verify the implementation specs — run this immediately after the artifact commit above and before the Stage Exit Protocol below, but only when `autoVerify` is effective for this stage.** Compute the effective setting exactly as the navigator does: a per-stage override in `autoVerifyStages` wins, else the global `autoVerify` (default off). When it is **off**, skip this whole step — the exit block's manual verify gate covers that path. When it is **on**, verify the implementation specs **now, in this session** (principle #2 applied to auto-verify: the digest and any fix decision land here, where the authoring context still exists — not deferred to a post-clear your session / start a fresh session navigator):
