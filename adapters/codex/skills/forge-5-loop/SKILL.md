...
6. **Close the handoff with the Stage Exit Protocol.** Finishing feature `{feature}` → starting the picked feature `{chosen}`'s PRD is a **cold** stage boundary (single-sourced in `references/stage-exit-protocol.md`). Feature `{feature}`'s impl-verify was already offered in Step 6.1, so step 1's gate self-suppresses when it ran or was skipped — the block then collapses to the clear your session / start a fresh session → next-command steps. Present it only when the chosen feature's PRD is absent (a PRD-present pick just runs its `nextCommand`):
...
1. **Verify feature {feature}'s loop first — if it isn't already verified.** When this stage has no
   ...
   **now, before clearing**. If verify already ran, is pending under auto-verify, or
   ...
   subagent):
   - **Verify feature {feature}'s loop now** *(recommended)* — dispatch the clean-room `forge-verifier`
   ...
   **Host / clean-room fallback:** if the question mechanism, the `Agent` tool, or the
   ...
   printing `/feature-forge:forge-verify {feature} impl` for the user to run inline/manually (mirroring
...
3. **Then run `/feature-forge:forge-1-prd {chosen}`** in the fresh session — or re-run `/feature-forge:forge`
